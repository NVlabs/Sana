...
├─ scripts/                   # launch_candidate, collect_run, create_model_experiment, per-model run_*.sh
├─ candidates/                # declarative manifests (pruned to live ones)
...
| `scripts/{launch_candidate,collect_run,create_model_experiment}.py` | `scripts/` | KEEP as-is |
| `candidates/*.toml` (live set) | `candidates/` | prune ~24 `*_fused_invariant` sweep residue |
...
same candidate/script + same seed, post-refactor output equals the pre-refactor **golden**
...
- [ ] every live `candidates/*.toml` still validates: `launch_candidate.py <c> --mode dry-run`.
...
|---|---|---|---|
| Wan-5B (1 GPU) | `candidates/wan22_ti2v_5b_baseline.toml` | `candidates/wan5b_kernel_easycache_pisa.toml` | ~2.885× |
| Wan-14B (1 GPU) | `candidates/wan22_t2v_a14b_baseline.toml` | `candidates/wan22_t2v_a14b_singlegpu_opt.toml` | ~2.17× |
| LingBot (4 GPU) | `candidates/lingbot_video_baseline.toml` | `candidates/lingbot_video_cudnn_pisa_full.toml` | ~2.60× |
...
Run via `launch_candidate.py <c> --mode local` (or `--mode sbatch`), then `collect_run.py runs/<id>`.

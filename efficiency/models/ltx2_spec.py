...
# get_model_spec(transformer) resolves a live model the way
...
def _ltx2_prunable_segment(hidden, ctx):
    ...
    NOTE: refine when wiring the forward -- LTX-2 concatenates video + audio
    tokens; the prunable segment is the video patch tokens only. Until the exact
    layout is wired, default to the whole sequence (correct on the single-video
    path where the hidden passed to the block loop is the video stream)."""

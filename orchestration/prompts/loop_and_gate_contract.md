...
5. GATE — branch on the correctness mode in your technique scope; use no external
   ...
   - **Lossy / quality-gated (`cache`, `pisa`)**: run
     ...
     YOUR OWN built-in multimodal vision to inspect candidate frames beside the
   ...
   - **Lossless / correctness-defined (`kernel`, `topology`)**: run
     ...
     to confirm authenticity (a real run of this candidate, not a resubmitted or
     mismatched baseline); do not judge visual quality. Do NOT compare outputs at
     all—no bit/latent/fp-tolerance/LPIPS comparison.
6. Retain in your frontier only when the candidate is authentic, OFF-identity
   ...
   reason to reject a lossless candidate.
  ...
  "frontier_points": [
    {
      ...
      "performance": { "frontier_axis": "latency|peak_memory", "baseline_total_s": <frozen>, "candidate_total_s": <measured>, "speedup": <computed> },
...
run provenance, and view candidate frames for authenticity. It applies LPIPS

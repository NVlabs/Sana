...
Hook contract (per docs/plans/notes/v2-ltx2-transformer-audit.md):
...
if BlockAdapterRegister is not None:
    ...
    # Registration fires at module import time; importing this module before
    # ``BlockAdapterRegister.is_supported`` runs is sufficient.
    ...
    # the same adapter factory is used whether or not FSDP is active. The
    # factory accesses `pipe.transformer.transformer_blocks` which the FSDP
    # wrapper forwards transparently.

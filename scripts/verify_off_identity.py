#!/usr/bin/env python3
"""Verify the OFF==baseline frame identity invariant for a config run.

A config that has no acceleration env (the OFF-identity check) must produce
...
Usage:
  python scripts/verify_off_identity.py <config_run_dir> <baseline_run_dir>
"""
...
def main() -> int:
    ...
    if not cand_frames:
        print(f"FAIL: no config frames under {cand_dir}/outputs/frames", file=sys.stderr)
        return 1
    if not base_frames:
    ...
    if len(cand_frames) != len(base_frames):
        print(
            f"WARN: frame count differs: {len(cand_frames)} config vs "

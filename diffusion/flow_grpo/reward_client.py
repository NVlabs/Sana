...
class CosmosImageRewardClient:
    ...
    def _make_headers(
        self, replica_id: Optional[str] = None, extra: Optional[dict] = None
    ) -> dict:
        headers: dict = {}
        if self.token:
        ...
        while True:
            ...
            if resp.status_code == 200:
                ...
                return self.get_zero_rewards(
                    self.batch_size_dict.get("uuid", 1), target_device
                ), 0

    def get_zero_rewards(self, batch_size: int, target_device: torch.device) -> dict:
        weighted_rewards = torch.zeros(
            batch_size, dtype=torch.float32, device=target_device
        )
        return {
    ...
    def process_results(
        self, scores: dict, target_device: torch.device
    ) -> dict:
        rewards = {
            k: torch.as_tensor(v, device=target_device, dtype=torch.float32)
            for k, v in scores.items()
        }
        weighted = sum(
            rewards[k] * w for k, w in self.reward_fn.items() if k in rewards
        )
    ...
    def submit_task_async(
        ...
        return executor.submit(
            self.submit_task, prompts, images, reward_fn, extra_data
        )
    ...
    def fetch_results_async(
        ...
        return executor.submit(
            ...
            uuid, reward_type, target_device, replica_id, poll_interval,
        )
...
def _run_test(client: CosmosImageRewardClient, prompts, images, extra_data=None):
    ...
    for rtype in task["reward_types"]:
        ...
        while True:
            ...
            result = client.check_results(
                task["uuid"], rtype, replica_id=task["replica_id"]
            )
...
def _run_async_demo(client: CosmosImageRewardClient, prompts, images, extra_data=None):
    ...
    submit_future = client.submit_task_async(
        executor, prompts, images, extra_data=extra_data
    )
    gpu_time_1 = _simulate_gpu_work(gpu_work_time, "GPU work during submit")
    ...
    gpu_time_2 = _simulate_gpu_work(gpu_work_time, "GPU work during fetch")
...
if __name__ == "__main__":
    ...
    # ---- hpsv3 ----
        ...
        prompts = [
            "cute chibi anime cartoon fox, smiling wagging tail "
            "with a small cartoon heart above sticker",
        ] * BATCH_SIZE
        images = make_fake_images(batch_size=BATCH_SIZE)
        _run_test(client, prompts, images)
    ...
    # ---- pickscore ----
        ...
        prompts = [
            "An astronaut's glove floating in zero-g "
            "with \"NASA 2049\" on the wrist",
        ] * BATCH_SIZE
        images = make_fake_images(batch_size=BATCH_SIZE)
        _run_test(client, prompts, images)

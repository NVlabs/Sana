#!/usr/bin/env python3
"""Strict soundness audit for model-agnostic efficiency config.

This is the CPU/static gate before GPU fanout. It proves that each config has
...
def audit_config(
    ...
    if not cid:
        fail(problems, path, "missing config id")
        ...
        if cid in {"attention_broadcast", "block_layer_feature_cache"}:
            ...
            if techniques != ["payload_cache"]:
                fail(
                    problems,
                    path,
                    "payload-cache config must compose as payload_cache, not whole-step step_cache",
                )
            ...
            if payload_mode == "pab":
                if env.get("SGLANG_HQ_PAYLOAD_CACHE_MODE") != "pab":
                    fail(problems, path, "PAB payload-cache config must export SGLANG_HQ_PAYLOAD_CACHE_MODE=pab")
                if cid == "attention_broadcast":
                ...
                fail(problems, path, "scheduled payload-cache config must expose SGLANG_HQ_PAYLOAD_CACHE_SKIP")
            cosmos_stage = (
                ROOT
            ...
            if (
                ...
                fail(
                    problems,
                    path,
                    "Cosmos3 builder must record scheduled/PAB payload-cache config in the efficiency Plan",
                )
        ...
        if dim == "token_prune":
            if not env.get("SGLANG_HQ_TOKEN_PRUNE_STEPS"):
                fail(
                    problems,
                    path,
                    "token_prune config will not install in Cosmos3 without SGLANG_HQ_TOKEN_PRUNE_STEPS",
                )
            method = (
                data.get("efficiency", {})
                .get("params", {})
        ...
        if cid == "teacache_signal_reuse":
            ...
            if data.get("efficiency", {}).get("name") != "teacache_residual":
                fail(
                    problems,
                    path,
                    "TeaCache config must use teacache_residual rather than whole-step teacache",
                )
            if data.get("env", {}).get("SGLANG_HQ_TEACACHE_REPLAY") != "block_residual":
                fail(
                    problems,
                    path,
                    "TeaCache config must export SGLANG_HQ_TEACACHE_REPLAY=block_residual",
                )
    ...
    if verification.get("mode") != "gpu":
        fail(problems, path, "verification.mode must be gpu for config baselines")
...
def main() -> int:
    ...
    for path in paths:
        cid, config_problems, config_warnings = audit_config(
        ...
        print(("FAIL" if config_problems else "PASS") + f" {path.relative_to(ROOT)}")
    ...
    for cid, count in sorted(seen.items()):
        if count > 1:
            problems.append(f"duplicate config id {cid!r}: {count} files")
    ...
    print(f"\n=== config soundness audit: {report['status']} ({len(paths)} config) ===")
    if problems:

...
This is a mathematical/algorithmic **lossless** technique. A valid candidate
...
candidate with both contracts below:
...
Profile first, then choose one measured bottleneck per candidate. In scope:
...
Do not assume every named degree multiplies independently. For every candidate,
...
candidate, but the dispatch must remain candidate-id-specific and fail closed for
unknown combinations. Create a new candidate manifest rather than relabeling the
...
- async buffers remain live through completion and stream/event dependencies are
  valid;
- every requested rank participates and the candidate has zero silent fallback.
...
For each full candidate run, copy the candidate-specific preflight result to
...
the exact `candidate_id` and run-directory basename as `run_id`. A
coordinator-level preflight claim without this run-local snapshot is not durable
candidate evidence.

### Bounded search loop

Hard budget: **20 candidate rounds**. One round is one hypothesis, one isolated
...
3. Implement exactly one guarded candidate and one candidate manifest.
4. Run the distributed preflight, then launch the full frozen workload through
   `scripts/launch_candidate.py` on the same Slurm resource envelope.
...
7. Retain only a mathematically equivalent candidate that improves the primary
...
preflight, manifest, and trace—must carry the same exact `candidate_id` and
...
- candidate manifest and activation;
- topology manifest, trace, preflight, and equivalence artifacts;
- frozen-baseline and candidate timing in the same scope;
- numeric frozen-baseline total, candidate total, recomputed speedup, per-rank

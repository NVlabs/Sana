...
def _resolve_group_size(group_size=None):
    group_size = _env_int("SOL_ATTN_GROUP_SIZE", GROUP_SIZE) if group_size is None else int(group_size)
    if group_size <= 0 or group_size & (group_size - 1):
        raise ValueError(f"SOL Attention group_size must be a positive power of two, got {group_size}")
    return group_size
...
def sol_attention(
    ...
    kv_block_size = block_size if kv_block_size is None else kv_block_size
    group_size = _resolve_group_size(group_size)
    num_warps = _env_int("SOL_ATTN_NUM_WARPS", 4) if num_warps is None else int(num_warps)
    num_stages = _env_int("SOL_ATTN_NUM_STAGES", 2) if num_stages is None else int(num_stages)

#!/usr/bin/env python3
"""Single-executor/Codex-visual-evaluation loop for Sana PISA optimization.

This is intentionally not a generic workflow language. It is the first-stage
centralized state machine for workflow uid `attention_pa`; all node
implementations are owned under `workflow/attention_pa/nodes/`.
"""
...
def transition(state: dict[str, Any], phase: str, result: NodeResult) -> str:
    ...
    if phase == "visual_review":
        ...
        state["resume_followups"] = [
            ...
            "Treat visual-review launch, attachment, LPIPS, malformed verdict, or missing-frame failures as evaluation infrastructure, not PISA quality evidence.",
            "Do not call Gemini or classify a recipe from missing Codex visual evidence.",
        ]
        return "write_resume"
    ...
    if phase == "check_eval":
        ...
        if result.outcome == "smooth":
            recipe_contract_ok = bool(state.get("recipe_contract_ok"))
            if executor_status == "complete" and recipe_contract_ok:
                return "delivery_gate"
            state["resume_target"] = "executor"
            state["resume_reason"] = "full_pisa_assessment_recorded_continue_single_executor"
            state["resume_followups"] = [
                "Use the completed full-run speed, LPIPS, blind Codex visual severity, dispatch, fallback, and attention trace to choose the next layer-by-step PISA density schedule.",
                "Keep dense attention for sensitive layer/step regions and lower exact-block density only where measured Sana evidence supports it.",
                "Maintain PISA-SEARCH-STATE.json with the parent trial, exact density, sparsity, layer policy, step policy, backend, and adaptation reason.",
                "Do not finish until PISA-RECIPES.json contains measured conservative, balanced, and aggressive recipes with full assessment evidence.",
                "When all three recipe tiers are complete and consistent, write AGENT-STATUS.json with status=complete.",
            ]
            return "write_resume"
        ...
        state["resume_followups"] = [
            "Produce, repair, or retry a completed full Sana Video diffusion run for the active PISA candidate before exiting again.",
            ...
            "Do not substitute single-DiT, module, or microbench evidence for PISA recipe quality classification.",
    ...
    if phase == "delivery_gate":
        if result.outcome == "published":
            ...
            state["terminal_reason"] = "pisa_frontier_delivery_published"
            return "done"
        state["resume_target"] = "executor"
        state["resume_reason"] = result.message or "pisa_delivery_invalid"
        state["resume_followups"] = [
            ...
            "Treat 10% exact-block density only as a starting prior; retain measured case-specific schedules.",
...
def run_loop(args: argparse.Namespace) -> dict[str, Any]:
    ...
    state_path = resolve_path(worktree, args.state_file, worktree / "state" / "workflow-attention_pa-state.json")
    event_log = resolve_path(worktree, args.event_log, worktree / "state" / "workflow-attention_pa-events.jsonl")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--experiment-uid", help="UID like sana-attention_pa-0001")

"""Runner for the G512 owner-warp ballot/direct-scatter diagnostic."""
...
KERNEL_PATH = ROOT / (
    ...
    "native_bf16_lean6_routeidx_g512_cursor_ballotscatter_fusedroute_n128_pair_tmemp_hybrid_"
    "massreuse_terminalo_fwd.py"
)
KERNEL_SHA256 = "9930d201104ed6bc035c670283bed08da3ea8114591ceea4c5873ede0caee106"
...
def _validate_kernel_identity() -> str:
    ...
    if observed != KERNEL_SHA256:
        raise RuntimeError(
            f"G512-cursor-ballotscatter/N128 kernel SHA mismatch: {observed} != "
            f"{KERNEL_SHA256}"
        )
    return observed
...
# CuTe DSL compilation is shape-specialized and expensive (~10s). Cache the
# compiled op by shape so a diffusion generation (thousands of same-shape attention
# calls) compiles once, not per call. The compiled op is invoked with fresh tensors
# and stream at call time, so caching by shape is safe.
_COMPILED_OP_CACHE: dict = {}


def make_native_bf16_lean6_routeidx_g512_cursor_ballotscatter_fusedroute_n128_pair_tmemp_hybrid_massreuse_terminalo_runner(
    T: int,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    *,
    ...
    from kernels.sol_attn_sm100.native_bf16_lean6_routeidx_g512_cursor_ballotscatter_fusedroute_n128_pair_tmemp_hybrid_massreuse_terminalo_fwd import (
        build_sol_attn_sm100_lean6_routeidx_g512_cursor_ballotscatter_fusedroute_n128_pair_tmemp_bf16_fwd,
        build_sol_attn_sm100_lean6_routeidx_g512_cursor_ballotscatter_fusedroute_n128_pair_tmemp_bf16_trace_fwd,
    )

    op = (
        build_sol_attn_sm100_lean6_routeidx_g512_cursor_ballotscatter_fusedroute_n128_pair_tmemp_bf16_trace_fwd(T)
        if trace_route_masks
        else build_sol_attn_sm100_lean6_routeidx_g512_cursor_ballotscatter_fusedroute_n128_pair_tmemp_bf16_fwd(T)
    )
    output_storage = None
    lse_storage = None
    if guard_elements:
    ...
    # torch>=2.8 may surface a generic ``torch.Stream`` (no ``.cuda_stream``) as the
    # current stream when the caller runs under a stream context (e.g. diffusers /
    # torch.compile). Fetch the raw CUDA stream handle robustly across versions.
    ...
    _cache_key = (b, h, T, num_blocks, guard_elements, bool(trace_route_masks))
    compiled_op = _COMPILED_OP_CACHE.get(_cache_key)
    if compiled_op is None:
        ...
        _COMPILED_OP_CACHE[_cache_key] = compiled_op
    else:
        compile_s = 0.0
    ...
    run.codegen_parent_kernel_source_sha256 = (
        "7723c2ac9ecd2e840182ed2f9d171679536d42be42a7b0a64ca7bdbee68ea47c"
    )
    ...
    run.codegen_diagnostic_axis = (
        "owner_warp_ballot_mask_plus_selected_lane_prefix_scatter"
    )
    run.phase_graph_changed = False
    run.math_changed = False
    run.shape_or_density_fast_path = False
    run.new_cross_warp_handoff = False
    run.new_named_or_cta_barrier = False
    run.route_mask_generation = "four_vote_ballot_sync_no_shuffle_tree"
    run.route_index_compaction = (
        "all_selected_lanes_single_rank_addressed_scatter"
    )
    ...
    run.route_index_reuse_hb = (
        "selected_owner_lanes_write_unique_rank_addressed_slots; existing_preexact_"
        "full_cta_join_publishes_to_warp5; warp5_reads_before_final_score_"
        "completion; owners_read_before_final_exact_p_namedbarrier; next_"
        "group_writers_reuse_only_after_exact_loop"
    )
...
__all__ = [
    "KERNEL_SHA256",
    "make_native_bf16_lean6_routeidx_g512_cursor_ballotscatter_fusedroute_n128_pair_tmemp_hybrid_massreuse_terminalo_runner",
]

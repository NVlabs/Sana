...
@triton.jit
def _ltx2_qknorm_pair_inplace_kernel(
    q_ptr,
    k_ptr,
    q_weight_ptr,
    k_weight_ptr,
    ...
    q_mask = (row < q_rows) & col_mask
    k_mask = (row < k_rows) & col_mask
    q_base = row * hidden + cols
    k_base = row * hidden + cols

    q = tl.load(q_ptr + q_base, mask=q_mask, other=0.0).to(tl.float32)
    k = tl.load(k_ptr + k_base, mask=k_mask, other=0.0).to(tl.float32)
    qw = tl.load(q_weight_ptr + cols, mask=col_mask, other=0.0).to(tl.float32)
    kw = tl.load(k_weight_ptr + cols, mask=col_mask, other=0.0).to(tl.float32)

    q_var = tl.sum(q * q, axis=0) / hidden
    k_var = tl.sum(k * k, axis=0) / hidden
    q_rstd = tl.rsqrt(q_var + eps)
    k_rstd = tl.rsqrt(k_var + eps)

    tl.store(q_ptr + q_base, q * q_rstd * qw, mask=q_mask)
    tl.store(k_ptr + k_base, k * k_rstd * kw, mask=k_mask)
...
def ltx2_qknorm_pair_inplace(
    ...
    grid = (max(q_rows, k_rows),)
    _ltx2_qknorm_pair_inplace_kernel[grid](
        q,
        k,
        q_weight,
        k_weight,

"""Claude49 per-column score-gate reformulation of claude42 packedsel.

Only packed-select inventory sites 1 and 2 change.  Each physical N128 score
tile is represented by two 64-column masks built once in dedicated SMEM and
published by an existing named barrier.  Owner
rows load those shared gates instead of rebuilding cohort/tail/exact predicates
for every score element.  Row validity is one gate per owner row.

The pinned CuTe stack has no packed float min/max.  Masks therefore use the
specified ``{0, -inf}`` additive form and are applied with the installed
``add_packed_f32x2`` helper.  This retains the documented ``-0.0 + 0.0`` edge;
the mandatory device bitwise gate decides it without a relaxed fallback.  No
reduction, shuffle, ballot, barrier, pipeline, topology, routing decision,
online recurrence, or production dispatch registration changes.
"""
...
MECHANISM_FAMILY = "sm100-bf16-g256-packedsel-colmask-v1"
CANDIDATE_AXIS = "per_column_additive_mask_replaces_select_chains"
CLAUDE49_PARENT_SOURCE_SHA256 = (
    "2adfa708e5d876d144d5f8844c28d4af6d46cdb09ba662599ba7ee4d1482b0cd"
)
COLMASK_IMPLEMENTED_SITES = (
    "route_approx_two_n64_column_gates_built_during_tile_generation",
    "exact_pair_two_n64_column_gates_built_once_per_pair",
    "row_validity_materialized_once_per_owner_row",
)
COLMASK_SIGNED_ZERO_POLICY = (
    "additive_zero_mask_exposes_negative_zero_to_mandatory_bitwise_gate"
)
CLAUDE42_PARENT_SOURCE_SHA256 = (
    "ffc9c49f97963707b332f1a939f6fafe886e423a9ddad4c19eb2c4e93cf0aeee"
)
PACKEDSEL_IMPLEMENTED_SITES = (
    "route_reduction_pair_trees_packed_arithmetic",
    "route_mass_paired_minmax_mul_f32x2",
    "exact_pair_paired_valid_minmax",
)
PACKEDSEL_UNAVAILABLE_INTRINSICS = (
    "packed_f32x2_compare_select_minmax",
    "packed_integer_minmax",
    "packed_predicate_compare_select",
)
...
G256_CURSOR_BALLOTSCATTER_PACKEDGMEMO_TWOROWHOIST_INLINEU32_FUSEDROUTE_N128_TMEMP_RECEIPT = {
    "claude49_parent_source_sha256": CLAUDE49_PARENT_SOURCE_SHA256,
    ...
    while logical_group_idx < num_logical_groups:
            ...
            if is_owner:
                ...
                for pair_idx in cutlass.range_constexpr(
                    ...
                    scaled0, scaled1 = cute.arch.mul_packed_f32x2(
                        ...
                        (Float32(SCALE_LOG2), Float32(SCALE_LOG2)),
                    )
                ...
                if has_route_approx:
                    ...
                    # remains the parent's later route-mass scratch.
        ...
        if is_owner and has_pair_exact:
            ...
            for pair_idx in cutlass.range(pair_count, unroll=1):
                ...
                # Keep claude42's installed integer min/max lowering and
...
class SolAttentionSm100Lean6RouteidxG256CursorBallot4FusedrouteN128PairTmempBf16Forward:
...
def build_sol_attn_sm100_lean6_routeidx_g256_cursor_ballotscatter_packedgmemo_tworowhoist_inlineu32_fusedroute_n128_pair_tmemp_bf16_fwd(
    T: int,
    *,
    is_causal: bool = False,
) -> SolAttentionSm100Lean6RouteidxG256CursorBallot4FusedrouteN128PairTmempBf16Forward:
    """Build the ordinary no-trace full-grid sibling."""

    return SolAttentionSm100Lean6RouteidxG256CursorBallot4FusedrouteN128PairTmempBf16Forward(
        T, is_causal=is_causal
    )


def build_sol_attn_sm100_lean6_routeidx_g256_cursor_ballotscatter_packedgmemo_tworowhoist_inlineu32_fusedroute_n128_pair_tmemp_bf16_trace_fwd(
    T: int,
    *,
    is_causal: bool = False,
) -> SolAttentionSm100Lean6RouteidxG256CursorBallot4FusedrouteN128PairTmempBf16Forward:

...
LAUNCH_PATH = ROOT / "scripts" / "launch_candidate.py"
COSMOS3_PATH = (
    ROOT
...
BACKEND_SELECTION_MANIFEST_PATH = (
    ROOT / "candidates/kwl_fusion/backend_selection_probe.toml"
)
MODELOPT_FP4_PATH = (
    ROOT
...
PIECEWISE_PISA_MANIFEST_PATH = (
    ROOT / "candidates/sparse_attention/piecewise_pisa_env.toml"
)
...
def main() -> int:
    ...
    launch = load_module(LAUNCH_PATH, "launch_candidate_unit")
    ...
    audit_rows = {row["candidate"]: row for row in audit.audit()[0]}
    check(
    ...
    local_baseline_note_paths = {
        "adaptive_delta_forecast": ROOT / "candidates/step_cache/adaptive_delta_forecast.toml",
        "scheduled_step_reuse": ROOT / "candidates/step_cache/scheduled_step_reuse.toml",
        "feature_norm_prune": ROOT / "candidates/token_prune/feature_norm_prune.toml",
        "region_dynamic_density": ROOT / "candidates/token_prune/region_dynamic_density.toml",
    }
    check(
    ...
    adapter_probe_note_paths = {
        ...
        / "candidates/kwl_fusion/backend_selection_probe.toml",
        "compile_graph_capture": ROOT
        / "candidates/kwl_fusion/compile_graph_capture.toml",
    }
    check(
    ...
    kwl_no_delta_paths = {
        "env_flag_kwl_bundle": ROOT / "candidates/kwl_fusion/env_flag_kwl_bundle.toml",
        "gemm_epilogue_fusion": ROOT / "candidates/kwl_fusion/gemm_epilogue_fusion.toml",
        "layout_copy_elimination": ROOT
        / "candidates/kwl_fusion/layout_copy_elimination.toml",
        "norm_modulation_residual_fusion": ROOT
        / "candidates/kwl_fusion/norm_modulation_residual_fusion.toml",
    }
    check(
    ...
    blocked = set(launch.cosmos3_blocked_candidate_ids())
    expected_blocked = {
        row["candidate"]
        for row in audit_rows.values()
        if row["cosmos3_status"] in audit.COSMOS3_GPU_UNSUPPORTED_STATUSES
    }
    check("launcher blocklist entries are candidate ids", blocked <= set(audit.ALIGNMENT))
    ...
    nvfp4_rows = nvfp4_result["candidate_manifest_alignment"]
    check(
    ...
    teacache_row = next(
        row for row in audit.audit()[0] if row["candidate"] == "teacache_signal_reuse"
    )
    check(
        ...
        and teacache_result["candidate_manifest_alignment"]["matches_public_cosmos_profile"]
    ...
    tome_row = next(
        row for row in audit.audit()[0] if row["candidate"] == "tome_merge_restore"
    )
    ...
    check(
        ...
        and not pab_result["candidate_manifest_alignment"]["scheduled_step_reuse"][
            "matches_public_pab"
        ]
        and not pab_result["candidate_manifest_alignment"]["adaptive_delta_forecast"][
            "matches_public_pab"
        ]
        and pab_result["candidate_manifest_alignment"]["attention_broadcast"][
            "matches_public_pab_controller"
        ]
        and pab_result["candidate_manifest_alignment"]["block_layer_feature_cache"][
            "matches_public_pab_controller"
        ]
    ...
    pisa_row = next(
        row for row in audit.audit()[0] if row["candidate"] == "piecewise_pisa_env"
    )
    ...
    token_prune_alignment = token_prune_behavior["candidate_manifest_alignment"]
    token_prune_rows = {
        row["candidate"]: row
        for row in audit.audit()[0]
        if row["candidate"]
    ...
    svg_row = next(
        row for row in audit.audit()[0] if row["candidate"] == "semantic_permutation"
    )
    check(
        ...
        and svg_result["candidate_manifest_alignment"][
            "matches_public_cosmos_sap_hyperparams"
        ]
        and svg_result["candidate_manifest_alignment"]["matches_pure_sap_plan"]
        and "identify_dynamic_map"
        in svg_result["candidate_manifest_alignment"]["pure_sap_algorithm_steps"]
        and not svg_result["candidate_manifest_alignment"][
    ...
    kwl_rows = {
        row["candidate"]: row
        for row in audit.audit()[0]
        if row["candidate"] in {"backend_selection_probe", "compile_graph_capture"}
    }
    check(
        ...
        and not kwl_result["candidate_manifest_alignment"][
    ...
    check(
        ...
        and sparse_policy_behavior["candidate_manifest_alignment"][
            "spatial_temporal_head_routing"
        ]["matches_public_core"]
        and sparse_policy_behavior["candidate_manifest_alignment"][
            "spatial_temporal_head_routing"

#!/usr/bin/env python3
"""Audit public-reference alignment claims for efficiency config.

This is deliberately stricter than a URL liveness check. It records what each
config claims relative to its public/canonical references and prevents
...
def cosmos3_baseline_or_ltx2_note_problems(path: Path, notes: str) -> list[str]:
    """Rows with no new Cosmos3 delta must say why they are probes, not config."""
    problems: list[str] = []
    note = notes.lower()
...
def launcher_cosmos3_blocklist() -> set[str]:
    ...
    spec = importlib.util.spec_from_file_location("launch_config_for_audit", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import {module_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    if hasattr(module, "cosmos3_blocked_config_ids"):
        return set(module.cosmos3_blocked_config_ids())
...
def validate_pab_public_short_gpu_diagnostic(cid: str) -> list[str]:
    ...
    if metadata.get("config_id") != cid:
        errors.append(
            f"{cid}: diagnostic metadata config_id={metadata.get('config_id')!r}"
        )
...
def validate_svg_sample_mse_short_gpu_diagnostic(cid: str) -> list[str]:
    ...
    env_preview = ((metadata.get("config_dry_run") or {}).get("env_preview") or {})
...
def validate_cat_public_short_gpu_diagnostic(cid: str) -> list[str]:
    ...
    params = ((metadata.get("config_dry_run") or {}).get("runtime_config") or {}).get("params") or {}
...
def check_teacache_public_controller_profile(
    ...
    profile = probe["config_manifest_alignment"]
...
def check_pab_public_controller_profile(
    ...
    if params.get("mode") != "pab" or env.get("SGLANG_HQ_PAYLOAD_CACHE_MODE") != "pab":
        problems.append(
            f"{path.relative_to(ROOT)}: public PAB payload-cache config must run with mode='pab'"
        )
    probe = load_module(
        ROOT / "scripts/probe_public_pab_alignment.py",
        "probe_public_pab_alignment_audit",
    ).probe()
    alignment = probe["config_manifest_alignment"][cid]
...
def next_required_proof(alignment: Alignment) -> str:
    ...
    if gap == "pure_algorithm_adapter_missing":
        return (
            ...
            "fallback, then prove the config changes execution in a GPU smoke run"
        )
    if gap == "already_in_cosmos3_baseline_or_ltx2_specific_not_applicable":
        return (
            "do not submit a config GPU job as optimization evidence; either keep "
    ...
    if gap == "te_recipe_model_specific_adapter_semantics_mismatch":
        return (
            ...
            "are disabled in the manifest and must remain out of the config "
    ...
    if gap == "probe_requires_concrete_backend_policy":
        return (
            ...
            "criteria before treating GPU runtime as config evidence"
        )
...
def audit() -> tuple[list[dict[str, str]], list[str]]:
    ...
    for path in paths:
        ...
        if alignment.cosmos3_status.endswith("route_label_only") and purpose == "delivery":
            problems.append(
                f"{path.relative_to(ROOT)}: purpose=delivery but config is route-label-only"
            )
    ...
    if missing:
        problems.append(f"alignment entries without config manifests: {missing}")
    ...
    if missing_blocks:
        problems.append(
            f"launcher does not block unsupported Cosmos3 GPU config: {missing_blocks}"
        )
    if extra_blocks:
        problems.append(
            f"launcher blocks config not marked unsupported by public alignment: {extra_blocks}"
        )
...
def write_markdown(path: Path, rows: list[dict[str, str]], problems: list[str]) -> None:
    lines = [
        ...
        "config is a model-agnostic baseline, local pure policy, runtime",
        ...
        "  yet prove the intended optimization as config evidence.",
        ...
        "## Config Matrix",
        "",
        "| Config | Dimension | Purpose | Scope | Equivalence Claim | Public Equivalence Gap | Algorithm Boundary | True Blocker | Glue Policy | Next Required Proof | Public Role | Cosmos3 Status | Local Claim | Residual Risk |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in rows:
        lines.append(
            "| {config} | {dimension} | {purpose} | {scope} | {equivalence_claim} | {public_equivalence_gap} | {algorithm_boundary} | {true_blocker} | {model_specific_glue_policy} | {next_required_proof} | {public_role} | {cosmos3_status} | {local_claim} | {residual_risk} |".format(
...
def main() -> int:
    ...
    for row in rows:
        print(
            ("PASS" if not any(row["config"] in p for p in problems) else "FAIL")
            + f" {row['dimension']}/{row['config']} {row['scope']} {row['cosmos3_status']}"
        )
    print(f"\n=== public reference alignment: {report['status']} ({len(rows)} config) ===")
    if problems:

#!/usr/bin/env python3
"""Equal-token encoder/upsampler ablations for the LTX-2.5 Stage-2 refiner.

All variants consume either the five-second 121-of-124-frame profile or the
ten-second 241-of-243-frame profile from an exact 896x512/24-FPS MiniMax-H3
MP4.  Both profiles enter Stage 2 with the same spatial token grid; only the
temporal latent length changes.

* ``latent_upsampler_taehv``: 672x384 -> TAEHV -> learned latent x2.
* ``pixel_resize_taehv``: 1344x768 -> TAEHV; no latent upsampler.
* ``pixel_resize_vae``: 1344x768 -> original LTX VAE encoder; no latent
  upsampler.  TAEHV remains the decoder for all three variants.
"""
...
from sol_attention import STAGE2_TAUS, Stage2DenseAttention, Stage2SolAttention
...
SOURCE_HEIGHT = 512
try:
    SOURCE_FRAME_COUNT = int(os.environ.get("H3_LTX_SOURCE_FRAMES", "124"))
    FRAME_COUNT = int(os.environ.get("H3_LTX_OUTPUT_FRAMES", "121"))
except ValueError as exc:
    raise RuntimeError("H3_LTX source/output frame counts must be integers") from exc
if (SOURCE_FRAME_COUNT, FRAME_COUNT) not in {(124, 121), (243, 241)}:
    raise RuntimeError(
        "H3_LTX frame profile must be 124->121 (5s) or 243->241 (10s)"
    )
LATENT_FRAME_COUNT = (FRAME_COUNT - 1) // 8 + 1
WORLD_SIZE = 1
STAGE2_SIGMAS = (0.909375, 0.725, 0.421875, 0.0)
LORA_STRENGTH = 0.8
MAX_VIDEO_TOKENS = 65536
EXPECTED_VIDEO_TOKENS = LATENT_FRAME_COUNT * 24 * 42
TARGET_LATENT_SHAPE = (1, 128, LATENT_FRAME_COUNT, 24, 42)
...
VARIANT_CONFIGS = {
    "latent_upsampler_taehv": {
        "encoder": "taehv",
        "pixel_width": WIDTH // 2,
        "pixel_height": HEIGHT // 2,
        "latent_upsampler": True,
    },
    "pixel_resize_taehv": {
        "encoder": "taehv",
        "pixel_width": WIDTH,
    ...
    "pixel_resize_vae": {
        "encoder": "ltx_video_vae",
        "pixel_width": WIDTH,
        "pixel_height": HEIGHT,
        "latent_upsampler": False,
    },
    "latent_upsampler_vae": {
        "encoder": "ltx_video_vae",
        "pixel_width": WIDTH // 2,
        "pixel_height": HEIGHT // 2,
        "latent_upsampler": True,
...
class ResidentRefiner:
    """Keep the complete refiner fleet resident on the one measured GB200."""
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.attention_backend = str(getattr(args, "attention_backend", "sol"))
        if self.attention_backend not in {"dense", "sol"}:
            raise ValueError(f"unsupported video self-attention backend: {self.attention_backend}")
        self.variant_config = VARIANT_CONFIGS[self.variant]
        ...
        if self.attention_backend == "sol":
            self.sol_attention = Stage2SolAttention(
                self.transformer,
                isolate_sol_from_compile=self.compile_enabled,
            )
        else:
            self.sol_attention = Stage2DenseAttention(
                self.transformer,
                compiled_transformer=self.compile_enabled,
            )
        self.sol_backend = (
            _actual_sol_backend() if self.attention_backend == "sol" else "dense"
        )
        if self.attention_backend == "sol" and self.sol_backend != "cute_sm100":
        ...
        if bool(self.variant_config["latent_upsampler"]):
            self.spatial_upsampler = (
                upsampler_block._upsampler_builder.build(
                    device=self.device, dtype=self.dtype
                )
                .eval()
                .requires_grad_(False)
            )
        self.original_vae_encoder = None
        if self.variant_config["encoder"] == "ltx_video_vae":
            self.original_vae_encoder = (
                upsampler_block._encoder_builder.build(
                    device=self.device, dtype=self.dtype
                )
                .eval()
                .requires_grad_(False)
        ...
        # These 1344x768 profiles use the reviewed sequential TAEHV memblock
        # path; parallel TAEHV has not been validated for either duration.
        # Keep the reviewed sequential memblock path explicit and fail closed
        # in tae_encode/tae_decode by passing parallel=False.
        self.taehv_parallel = False
        ...
        if self.spatial_upsampler is not None:
            self.residency["spatial_upsampler"] = _module_residency(
                "spatial_upsampler", self.spatial_upsampler
            )
        if self.original_vae_encoder is not None:
            self.residency["original_ltx_video_vae_encoder"] = _module_residency(
                "original_ltx_video_vae_encoder", self.original_vae_encoder
            )
    ...
    def prepare_input(self, source_path: Path) -> tuple[torch.Tensor, dict[str, Any]]:
        ...
        pixel_height = int(self.variant_config["pixel_height"])
        pixel_width = int(self.variant_config["pixel_width"])
        pixels = video_preprocess(
            frames, pixel_height, pixel_width, self.dtype, self.device
        )
        expected_pixels = (1, 3, FRAME_COUNT, pixel_height, pixel_width)
        ...
        return pixels, {
            ...
            "encoder_input_width": pixel_width,
            "encoder_input_height": pixel_height,
            "encoder_pixels_shape": list(pixels.shape),
            "latent_upsampler": bool(self.variant_config["latent_upsampler"]),
        }

    def prompt_stats(self, prompt: str) -> dict[str, Any]:
        tokenizer = self.gemma.tokenizer
        if tokenizer is None:
            raise RuntimeError("resident Gemma tokenizer is unavailable")
        hf_tokenizer = tokenizer.tokenizer
        raw_ids = hf_tokenizer(
        ...
        kept_count = sum(
            int(weight)
            for _, weight in tokenizer.tokenize_with_weights(prompt)["gemma"]
        )
        max_tokens = int(tokenizer.max_length)
        stats = {
            "characters": len(prompt),
            "whitespace_words": len(prompt.split()),
            "raw_tokens_including_bos": raw_count,
            "kept_tokens": kept_count,
            "max_tokens": max_tokens,
            "truncated": raw_count > max_tokens,
        }
        if stats["truncated"]:
            raise ValueError(
                f"prompt requires {raw_count} Gemma tokens but the encoder accepts "
                f"only {max_tokens}; refusing a silently truncated experiment"
            )
        return stats
    ...
    def video_encode(self, pixels: torch.Tensor) -> torch.Tensor:
        if self.variant_config["encoder"] == "taehv":
            ...
            latent = self.taehv.encode_video(
                tae_pixels, parallel=False, show_progress_bar=False
            )
            expected = (
                1,
                16,
            ...
            if tuple(latent.shape) != expected:
                raise RuntimeError(
                    f"unexpected TAEHV latent shape {tuple(latent.shape)}, "
                    f"expected {expected}"
                )
            return latent.permute(0, 2, 1, 3, 4).contiguous()

        if self.original_vae_encoder is None:
            raise RuntimeError("original LTX video VAE encoder was not loaded")
        latent = self.original_vae_encoder.tiled_encode(
            pixels, TileSizeConfig.default()
        )
        expected = (
            1,
            128,
        ...
        if tuple(latent.shape) != expected:
            raise RuntimeError(
                f"unexpected original VAE latent shape {tuple(latent.shape)}, "
                f"expected {expected}"
            )
    ...
    def upsample(self, normalized: torch.Tensor) -> torch.Tensor:
        if self.spatial_upsampler is None:
            raise RuntimeError("latent upsampler is unavailable for this variant")
        raw = normalized.float() * self.latent_std + self.latent_mean
        upsampled_raw = self.spatial_upsampler(raw.to(self.dtype)).float()
        if tuple(upsampled_raw.shape) != TARGET_LATENT_SHAPE:
            raise RuntimeError(
                f"unexpected upsampled latent shape {tuple(upsampled_raw.shape)}, "
                f"expected {TARGET_LATENT_SHAPE}"
            )
    ...
    def _checked_attention_stats(self) -> dict[str, Any]:
        ...
        completed_steps = len(self.sigmas) - 1
        if self.attention_backend == "sol":
            expected = {
                "completed_steps": completed_steps,
                "dense_calls": completed_steps,
                "sol_calls": 47 * completed_steps,
            }
            expected_kernel_calls = 47 * completed_steps
            expected_backend = "cute_sm100"
        else:
            expected = {
                "completed_steps": completed_steps,
                "dense_calls": 48 * completed_steps,
                "sol_calls": 0,
            }
            expected_kernel_calls = 0
        ...
        if kernel_calls != expected_kernel_calls or self.sol_backend != expected_backend:
            raise RuntimeError(
                f"rank {self.rank} invalid {self.attention_backend} attention execution: "
    ...
    def run_sample(
        ...
        *,
        ...
        normalized, video_encode_s = _timed_cuda(
            lambda: self.video_encode(pixels)
        )
        del pixels
        if bool(self.variant_config["latent_upsampler"]):
            upscaled, latent_upsample_s = _timed_cuda(
                lambda: self.upsample(normalized)
            )
            del normalized
        else:
            if tuple(normalized.shape) != TARGET_LATENT_SHAPE:
                raise RuntimeError(
                    f"no-upsampler variant produced latent {tuple(normalized.shape)}, "
                    f"expected {TARGET_LATENT_SHAPE}"
                )
            upscaled = normalized
            latent_upsample_s = 0.0
        ...
        if self.rank == 0 and output_path is not None:
            try:
                ...
                output_info, output_verify_s = _timed_cuda(lambda: _verify_video(output_path))
        ...
        local = {
            ...
            "timings": {
                ...
                "output_verify_s": output_verify_s,
                "cleanup_s": cleanup_s,
        ...
        return {
            ...
            **phase_max,
            ...
            "parallelism": {
                "kind": "single_gpu_full_sequence",
                "degree": WORLD_SIZE,
                "parameter_replication": "single_resident_copy",
                "activation_layout": "unsharded_single_rank",
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--variant", choices=tuple(VARIANT_CONFIGS), required=True
    )
    ...
    parser.add_argument("--source-width", type=int, default=SOURCE_WIDTH)
    parser.add_argument("--source-height", type=int, default=SOURCE_HEIGHT)
    parser.add_argument("--source-frames", type=int, default=SOURCE_FRAME_COUNT)
    ...
    parser.add_argument(
        ...
        choices=("dense", "sol"),
        default="sol",
        help="change only video self-attention; cross/audio attention stay dense",
    )
...
def _validate_paths(args: argparse.Namespace) -> None:
    ...
    if not 1 <= args.measured_repeats <= 100:
        raise ValueError("measured repeat count must be in [1, 100]")
    if not 1 <= args.warmup_repeats <= 10:
        raise ValueError("warmup repeat count must be in [1, 10]")
    if args.compile:
...
def _output_name(
    ...
    *,
    ...
    source_named: bool,
    unique_source_index: bool,
) -> str:
    if not source_named:
        compile_tag = "compile" if compile_enabled else "eager"
        return f"{variant}-{compile_tag}-1344x768-{FRAME_COUNT}f.mp4"
    ...
    prefix = ""
    if unique_source_index:
        source_index = int(record.get("source_index", -1))
        if source_index < 0:
            raise ValueError("batched source-named output requires source_index")
        prompt_id = str(record.get("prompt_id") or "prompt")
        safe_prompt_id = "".join(
            character if character.isalnum() or character in "-_" else "_"
            for character in prompt_id
        ).strip("_")
        if not safe_prompt_id:
            safe_prompt_id = "prompt"
        prefix = f"{source_index:02d}-{safe_prompt_id}-"
    return f"{prefix}{safe_stem}-{variant}-1344x768-{FRAME_COUNT}f.mp4"
...
def main() -> int:
    ...
    try:
        ...
        profile = (
            f"minimax_h3_896x512_{SOURCE_FRAME_COUNT}f_to_ltx25_"
            f"1344x768_{FRAME_COUNT}f_"
            f"{args.variant}_single_gpu_v1"
        )
        warmup_outputs = [
            args.metadata_dir
            / f".warmup-{repeat:02d}-{args.variant}-1344x768-{FRAME_COUNT}f.mp4"
            for repeat in range(args.warmup_repeats)
        ]
            ...
            try:
                ...
                output_names = [
                    _output_name(
                        ...
                        unique_source_index=args.expected_samples > 1,
                    )
                ...
                stale_warmups = [str(path) for path in warmup_outputs if path.exists()]
                if stale_warmups:
                    raise FileExistsError(
                        "refusing to overwrite stale warm-up output: "
                        + ", ".join(stale_warmups)
                    )
        ...
        if rank == 0:
            _write_json(
                args.metadata_dir / "residency.json",
                {
                    ...
                    "parallelism": "single_gpu_full_sequence",
                    "parameter_replication": "single_resident_copy",
                    ...
                    "input_contract": {
                        ...
                        "source_fps": FPS,
                        "consumed_frames": FRAME_COUNT,
                        "encoder_backend": variant_config["encoder"],
                        "encoder_input_width": variant_config["pixel_width"],
                        "encoder_input_height": variant_config["pixel_height"],
                        "latent_upsampler": variant_config["latent_upsampler"],
                        "stage2_latent_shape": list(TARGET_LATENT_SHAPE),
                        "stage2_video_tokens": EXPECTED_VIDEO_TOKENS,
                        "output_width": WIDTH,
        ...
        # Warm the complete service path, including source-audio decode,
        # H.264 encode/mux and output verification.  The file is excluded from
        # measured artifacts and removed after all ranks leave run_sample().
        warmups: list[dict[str, Any]] = []
        for repeat, warmup_output in enumerate(warmup_outputs):
            warmup = models.run_sample(
                records[0], warmup=True, output_path=warmup_output
            )
            if rank == 0:
        ...
        if rank == 0:
            _write_json(
                args.metadata_dir / "warmup.json",
                {"count": len(warmups), "repeats": warmups},
            )
        ...
        for sample_index, record in enumerate(records):
            retained_output = args.output_dir / _output_name(
                record,
                variant=args.variant,
                compile_enabled=args.compile,
                source_named=args.source_named_outputs,
            ...
            if rank == 0:
                phase_means = {
                    phase: round(
                        sum(float(item[phase]) for item in repeats) / len(repeats), 6
                    )
                ...
                aggregate = {
                    ...
                    "index": record["index"],
                    "prompt_id": record.get("prompt_id"),
                    "prompt_stats": repeats[0]["prompt_stats"],
                    "seed": record["seed"],
                    "input": repeats[0]["input"],
                    "output": str(retained_output),
                    "output_info": repeats[0]["output_info"],
                    "measured_repeats": len(repeats),
                    "sample_wall_s": round(
                        sum(float(item["sample_wall_s"]) for item in repeats) / len(repeats),
                        6,
                    ),
                    "service_wall_s": round(
                        sum(float(item["service_wall_s"]) for item in repeats) / len(repeats),
                        6,
                    ),
                    "sample_wall_s_values": [item["sample_wall_s"] for item in repeats],
                    "service_wall_s_values": [item["service_wall_s"] for item in repeats],
                    "phases_s_mean": phase_means,
                    "repeats": repeats,
                }
                sample_aggregates.append(aggregate)
                _write_json(
        ...
        if rank == 0:
            ...
            service_wall_s_mean = round(
                sum(float(item["service_wall_s"]) for item in measurements)
                / len(measurements),
                6,
            )
            phases_s_mean = {
            ...
            outputs = [str(item["output"]) for item in sample_aggregates]
            sample_summaries = [
                {
                    "index": item["index"],
                    ...
                    "service_wall_s": item["service_wall_s"],
                }
                for item in sample_aggregates
            ]
            if len(sample_summaries) != len(records):
                raise RuntimeError("sample aggregate count does not match source samples")
            benchmark = {
                "schema_version": 2,
                "profile": profile,
                "variant": args.variant,
                ...
                "workload": {
                    ...
                    "source_fps": FPS,
                    "preprocess": (
                        f"take_first_{FRAME_COUNT}_frames_then_resize_and_center_crop"
                    ),
                    "encoder_backend": variant_config["encoder"],
                    "encoder_input_width": variant_config["pixel_width"],
                    "encoder_input_height": variant_config["pixel_height"],
                ...
                "timing_scope": (
                    "resident post-warmup sequential per-video service latency; "
                    "service_wall_s includes output verification and request cleanup; "
                    "model loading, torch.compile, Inductor/Triton autotuning, "
                    "Sol first-use compilation, and one complete encode/mux warm-up "
                    "request are excluded"
                ),
                "warmup_includes_h264_encode_mux": True,
                "warmup_repeats_excluded": args.warmup_repeats,
                "compile": _compile_metadata(args),
                "measured_hot_samples": len(measurements),
                "unique_samples": len(sample_aggregates),
                "measured_repeats_per_sample": args.measured_repeats,
                ...
                "service_wall_s_mean": service_wall_s_mean,
                "service_wall_s_min": min(
                    float(item["service_wall_s"]) for item in measurements
                ),
                "service_wall_s_max": max(
                    float(item["service_wall_s"]) for item in measurements
                ),
                ...
                benchmark.update(
                    {
                        ...
                        "service_wall_s": measurements[0]["service_wall_s"],
                        "phases_s": {
            ...
            success = {
                "status": "succeeded",
                "rendered_outputs": len(sample_aggregates),
                "measured_hot_samples": len(measurements),
                "unique_samples": len(sample_aggregates),
                "measured_repeats_per_sample": args.measured_repeats,
                "sample_wall_s_mean": sample_wall_s_mean,

...
- **Token-family coverage:** before declaring structured negative,
  compare at least five token-reduction families from
  `search_space/02_token_pruning.md`, including pruning, merging, masking,
  region-aware or dynamics-aware selection, and one of mediator tokens,
  cluster-aware pruning, context-token pruning, token-wise caching, or dynamic
  token-density control when applicable.
...
lossy generative token reduction, LPIPS and Gemini are joint ranking signals,

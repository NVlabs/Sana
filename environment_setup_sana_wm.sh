#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# SANA-WM environment installer. Single source of truth for deps is
...
#   bash ./environment_setup_sana_wm.sh           # install into the active env
#
...
# Pi3X (camera intrinsics from a single image): --no-deps so it doesn't
# downgrade torch/numpy.
pip install git+https://github.com/yyfz/Pi3.git --no-deps

# flash-attn needs torch present at build time.

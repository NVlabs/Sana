...
def _get_config(n_gpus=8, dataset="pickscore", reward_fn=None):
    ...
    config.nvfp4_skip_modules = [
        "t_embedder", "t_block", "cfg_embedder",
        "y_embedder", "x_embedder", "final_layer", "logvar_linear",
        "attn.qkv",
    ]
...
def _set_bon_combo(config, reward_name, best_of_n, num_image_per_prompt, n_gpus,
                   grad_steps_per_epoch, rollout_bsz, train_bsz, seed=42):
...
def _build_reward(reward_name, best_of_n, num_image_per_prompt):
    ...
    return _set_bon_combo(cfg, reward_name, best_of_n, num_image_per_prompt,
                          n_gpus=8, grad_steps_per_epoch=1,
                          rollout_bsz=rollout_bsz, train_bsz=16, seed=42)

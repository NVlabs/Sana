...
- Symptom: `speedup=None` for every config.
...
- Fix (`scripts/collect_run.py`): `--frame-count` now defaults to the run's own
  ...
  Baseline and every config use the identical policy, keeping pairs aligned.
...
`baseline_total_s=881.85, config_total_s=881.85, speedup=1.0,
...
- Observed during fan-out (run fanout_hunyuan_20260620T183315Z): kwl config
  ...
  fail/reject with a hallucinated artifact ("config shows an entirely
...
- Observed false-fail rate ~2/8 early config, all on near-identical
  (LPIPS<=~0.02) runs; the judge still correctly fails real cliffs (LPIPS ~0.95)
  and passes other low-LPIPS config. So it is a reliability caveat, not a
  total failure.
- Impact: speed numbers unaffected; mid-fan-out no winner is lost (agents retain
  on speed). Real risk is at FINAL selection — a genuinely good fast config

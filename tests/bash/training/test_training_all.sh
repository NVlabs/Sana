...
hf download Efficient-Large-Model/sana_data_public --repo-type dataset --local-dir ./data/data_public --local-dir-use-symlinks False
hf download Efficient-Large-Model/toy_data --repo-type dataset --local-dir ./data/toy_data --local-dir-use-symlinks False
hf download Efficient-Large-Model/video_toy_data --repo-type dataset --local-dir ./data/video_toy_data --local-dir-use-symlinks False

mkdir -p output/pretrained_models
hf download Efficient-Large-Model/Wan2.1-T2V-1.3B --repo-type model --local-dir ./output/pretrained_models/Wan2.1-T2V-1.3B --local-dir-use-symlinks False
...
bash train_video_scripts/train_video_ivjoint.py configs/sana_video_config/256ms/Sana_1600M_256px_AdamW_fsdp.yaml --np=2 --train.num_epochs=1 --train.log_interval=1 --train.train_batch_size=1

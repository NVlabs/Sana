...
#      provided by the spec, else refused (like BlockAdapterRegister.is_supported).
...
#      conflict). This proves the ABSENCE of *structural* conflicts; numerical
#      composition of lossy items is bounded empirically + the off==identity
#      invariant of each technique.
...
def check_conflicts(items: Sequence, horizon: int = 64) -> list[str]:
    ...
    for seam in EXCLUSIVE_SEAMS:
        ...
        # two techniques both writing an exclusive seam only clash if their
        # schedules overlap; transforms are always-on so always clash.
        clash = []

...
def main():
    ...
    if cache_method in {"off", "none", "0", "false"}:
        cache_method = ""
    if cache_method and (not context_parallel or world_size != 4):
        raise RuntimeError(
            "cache candidates must execute on the frozen four-rank Ulysses substrate "
    ...
    if context_parallel:
        ...
        if (ring_degree, ulysses_degree) not in ((1, 4), (2, 2)):
            raise ValueError(
                "certified Wan topology candidates are ring1/ulysses4 or "
    ...
    benchmark = {
        ...
        "baseline_class": "cache_candidate" if cache_method else "pristine_cp4_control",
        "config": run_config,
        "aggregate": {
            "total_s": total_median,
            "denoise_s": denoise_median,

...
dense by default; approximate them only as a separately identified candidate
...
- existing dense backend, available PISA kernel/backend, block-size constraints,
  and whether the candidate really dispatches sparse kernels;
...
```json
{
  ...
  "trials": [
    {
      "candidate_id": "<id>",
      "parent_candidate_id": "<id or empty>",

...
def pipeline_with_logprob_sana(
    ...
    *,
    ...
    if latents is None:
        ...
        latents = torch.randn(
            prompt_embeds.shape[0], num_channels, latent_size, latent_size,
    ...
    mask_4d = (prompt_attention_mask.unsqueeze(1).unsqueeze(1).to(torch.int16)
    ...
    if do_cfg:
        ...
        neg_mask_4d = (negative_prompt_attention_mask.unsqueeze(1).unsqueeze(1).to(torch.int16)
...
def _rollout_for_one_prompt(
    rollout_transformer, vae, num_channels, latent_size, reward_fn, executor,
    prompt_text, prompt_meta,
    prompt_embed_single, prompt_mask_single,
    neg_prompt_embed_single, neg_prompt_mask_single,
    prompt_token_ids_single,
    config, device,
    inference_models=None, transformer_ddp=None, peft_transformer=None,
):
    ...
    full_rollout_num = max(1, min(int(getattr(config.sample, "full_rollout_num", config.sample.best_of_n)), draft_total))
    ...
    if preview_step > 0:
        if _can_swap:
            current_transformer = _select_inference_transformer(preview_model_key, inference_models, transformer_ddp, peft_transformer)
        else:
        ...
        with torch.no_grad():
            for _ in range(config.sample.per_prompt_iter_num):
                ...
                init_latents = _prepare_latents_from_seeds(seed_list, num_channels, latent_size, latent_size, device, p_emb.dtype)
                images, _, _ = pipeline_with_logprob_sana(
                    current_transformer, vae, latents=init_latents, prompt_embeds=p_emb,
                    prompt_attention_mask=p_mask,
                    negative_prompt_embeds=neg_emb, negative_prompt_attention_mask=neg_mask,
                    num_inference_steps=preview_step,
                    guidance_scale=config.rollout_sample_guidance_scale,
                    noise_level=noise_level, deterministic=deterministic,
                    solver=solver,
                )
        ...
        stage1_idx = select_indices_by_mode(draft_rewards, full_rollout_num, getattr(config.sample, "stage1_select_mode", "best_worst"))
        selected_seeds = [seed_pool[int(i)] for i in stage1_idx.cpu().tolist()]

        if _can_swap and fullrollout_model_key != preview_model_key:
            current_transformer = _select_inference_transformer(fullrollout_model_key, inference_models, transformer_ddp, peft_transformer)
        ...
        for start in range(0, len(selected_seeds), full_chunks):
            ...
            neg_emb = neg_prompt_embed_single.repeat(bs, 1, 1)
            neg_mask = neg_prompt_mask_single.repeat(bs, 1)
            init_latents = _prepare_latents_from_seeds(chunk_seeds, num_channels, latent_size, latent_size, device, p_emb.dtype)
            with torch.no_grad():
        ...
        for _ in range(config.sample.per_prompt_iter_num):
            ...
            with torch.no_grad():
                images, all_latents, step_sigmas = pipeline_with_logprob_sana(
                    rollout_transformer, vae, latents=init_latents, prompt_embeds=p_emb,
                    prompt_attention_mask=p_mask,
                    negative_prompt_embeds=neg_emb, negative_prompt_attention_mask=neg_mask,
                    num_inference_steps=full_steps,
                    guidance_scale=config.rollout_sample_guidance_scale,
    ...
    keep = select_indices_by_mode(
        collated["rewards"]["avg"], config.sample.best_of_n,
...
def eval_fn(
    pipeline, eval_transformer, vae, num_channels, latent_size,
    test_dataloader, config, device, rank, world_size,
    ...
    test_sampler = DistributedSampler(test_dataloader.dataset, num_replicas=world_size, rank=rank, shuffle=False) if world_size > 1 else None
    eval_loader = DataLoader(test_dataloader.dataset, batch_size=config.sample.test_batch_size,
                             sampler=test_sampler, collate_fn=test_dataloader.collate_fn, num_workers=0)
    ...
    for test_batch in tqdm(eval_loader, desc="Eval:", disable=not is_main_process(rank)):
        ...
        with torch.no_grad():
            prompt_embeds, prompt_mask, neg_embeds, neg_mask = pipeline.encode_prompt(
                prompt=prompts, negative_prompt="", device=device,
                max_sequence_length=TEXT_ENCODER_MAX_SEQ_LEN, do_classifier_free_guidance=True,
            )
            images, _, _ = pipeline_with_logprob_sana(
                eval_transformer, vae,
                num_channels=num_channels, latent_size=latent_size,
                prompt_embeds=prompt_embeds, prompt_attention_mask=prompt_mask,
                negative_prompt_embeds=neg_embeds, negative_prompt_attention_mask=neg_mask,
...
def main(_):
    ...
    if is_main_process(rank):
        ...
        wandb.init(project="sol-rl", name=config.run_name, config=config.to_dict(),
                   resume="allow", dir=log_dir, id=config.run_name)
    ...
    pipeline = SanaPipeline.from_pretrained("Efficient-Large-Model/Sana_1600M_1024px_BF16_diffusers", torch_dtype=torch.bfloat16)
    ...
    for mtype in sorted(needed_model_types):
        ...
        if "nvfp4" in mtype:
            ...
            n_rep, n_skip, rep_d, skip_d = replace_linear_with_te(m, skip_modules=nvfp4_skip_modules, min_dim=nvfp4_min_dim, replace_conv1x1=True)
    ...
    if config.use_lora:
        ...
        lora_cfg = LoraConfig(
            r=config.train.lora_rank, lora_alpha=config.train.lora_alpha,
            init_lora_weights=init_lora_weights, target_modules=list(config.train.lora_target_modules),
        )
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        if epoch % config.eval_freq == 0 and not config.debug:
            ...
            eval_fn(pipeline, peft_transformer, vae, num_channels, latent_size,
                    test_dataloader, config, device, rank, world_size,
                    global_step, eval_reward_fn, executor, mixed_precision_dtype, ema, transformer_trainable_parameters)
            random.setstate(py_rng); np.random.set_state(np_rng)
            torch.random.set_rng_state(torch_rng); torch.cuda.set_rng_state_all(cuda_rng)
        ...
        txt_tokens = pipeline.tokenizer(
            prompts, max_length=TOKENIZER_MAX_LENGTH, padding="max_length",
        ...
        for prompt_idx in tqdm(range(config.sample.per_gpu_to_process_prompts),
                               desc=f"Epoch {epoch}: rollout", disable=not is_main_process(rank), dynamic_ncols=True):
            collated_prompt_samples, final_images, final_prompts = _rollout_for_one_prompt(
                ...
                vae=vae, num_channels=num_channels, latent_size=latent_size,
                reward_fn=reward_fn, executor=executor,
                prompt_text=prompts[prompt_idx], prompt_meta=prompt_metadata[prompt_idx],
                ...
                inference_models=inference_models, transformer_ddp=transformer_ddp,
        ...
        save_step_reward_groups(config=config, global_step=global_step, epoch=epoch,
                                rank=rank, world_size=world_size, prompt_reward_groups=step_prompt_reward_groups)
        ...
        if is_main_process(rank):
            r2l = gathered_rewards_dict["avg"].reshape(world_size * config.sample.per_gpu_to_process_prompts, -1, num_train_timesteps).mean(axis=-1)
            wandb.log({"epoch": epoch, "reward/mean": r2l.mean(), "reward/max": r2l.max(axis=1).mean(),
                        "reward/min": r2l.min(axis=1).mean(), "reward/range": r2l.max(axis=1).mean() - r2l.min(axis=1).mean()}, commit=False)
        ...
        if stat_tracker is not None:
            ...
            if is_main_process(rank):
                ...
                wandb.log({"group_size": gs, "trained_prompt_num": tp, "zero_std_ratio": zsr, "reward_std_mean": rsm,
                            "mean_reward_100": stat_tracker.get_mean_of_top_rewards(100),
                            "mean_reward_50": stat_tracker.get_mean_of_top_rewards(50)}, commit=False)
            stat_tracker.clear()
        else:
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            perms_time = torch.stack([torch.randperm(num_timesteps_filtered, device=device) for _ in range(total_batch_size_filtered)])
            for key in ["timesteps", "next_timesteps"]:
                shuffled[key] = shuffled[key][torch.arange(total_batch_size_filtered, device=device)[:, None], perms_time]
            ...
            for train_batch in tqdm(batches, desc=f"Epoch {epoch}.{inner_epoch}: train",
                                    disable=not is_main_process(rank), dynamic_ncols=True):
                ...
                for j_idx in range(num_train_timesteps):
                    ...
                    loss_terms = {}
                    advantages_clip = torch.clamp(train_batch["advantages"][:, j_idx],
                    ...
                    if hasattr(config.train, "adv_mode"):
                        if config.train.adv_mode == "positive_only":
                            ...
                            advantages_clip = torch.where(advantages_clip > 0, torch.ones_like(advantages_clip), torch.zeros_like(advantages_clip))
                    ...
                    with torch.no_grad():
                        weight_factor = torch.abs(x0_prediction.double() - x0.double()).mean(
                            dim=tuple(range(1, x0.ndim)), keepdim=True).clip(min=1e-5)
                    ...
                    with torch.no_grad():
                        neg_wf = torch.abs(negative_x0_prediction.double() - x0.double()).mean(
                            dim=tuple(range(1, x0.ndim)), keepdim=True).clip(min=1e-5)
                    ...
                    loss_terms["old_kl_div"] = torch.mean(((old_prediction - ref_forward_prediction) ** 2).mean(dim=tuple(range(1, x0.ndim)))).detach()
                    ...
                    if current_accumulated_steps % effective_grad_accum_steps == 0:
                        ...
                        log_info["grad_norm"] = grad_norm.detach().float().item() if torch.is_tensor(grad_norm) else float(grad_norm)
                        ...
                        if is_main_process(rank):
                            _log_dict = {"global_step": global_step, "gradient_update_times": gradient_update_times,
                                        "epoch": epoch, "inner_epoch": inner_epoch,
                                        "current_time": time.time() - start_time, **reduced}

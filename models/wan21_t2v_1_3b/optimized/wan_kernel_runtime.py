...
class WanKernelRuntime:
    ...
    def __init__(self, pipe: Any, output_dir: Path):
        ...
        self.active_config = os.environ.get("WAN22_KERNEL_ACTIVE_CANDIDATE", "").strip()
        self.ledger_enabled = _enabled("WAN22_KERNEL_LEDGER") or bool(self.stack)
        self.pair_enabled = _enabled("WAN22_KERNEL_PAIR_BENCH") and bool(self.active_config)
        ...
        self.activation: dict[str, Any] = {
            ...
            "active_config": self.active_config or None,
            "methods": [],
    ...
    def _set_active_config(self, enabled: bool) -> None:
        if self.active_config == "regional_compile":
            ...
            return
        if self.active_config == "qkv_fusion":
            ...
            return
        if self.active_config in {"invariant_cache", "invariant_cache_v2"}:
            self.invariant_cache_enabled = enabled
            return
        if self.active_config == "cross_kv_cache":
            self.cross_kv_cache_enabled = enabled
            return
        if self.active_config == "bf16_output_glue":
            self._set_bf16_output_glue(enabled)
            return
        if self.active_config == "native_cudnn_attention":
            self._set_native_cudnn_attention(enabled)
            return
        if self.active_config == "native_flash_attention":
            self._set_native_flash_attention(enabled)
            return
        raise RuntimeError(f"No paired toggle for active config {self.active_config!r}")
    ...
    def run_paired_dit_benchmark(self) -> dict[str, Any] | None:
        ...
        self._set_active_config(False)
        self._time_direct_dit()
        self._set_active_config(True)
        self._time_direct_dit()
        ...
        for order in orders:
            for enabled in order:
                self._set_active_config(enabled)
        ...
        payload = {
            ...
            "comparison_scope": (
                ...
                if self.active_config == "qkv_fusion" and self._compiled_impls
                else "active_config_OFF_ON"
            ),

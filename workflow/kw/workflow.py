#!/usr/bin/env python3
"""Direct executor/eval/reviewer loop for the KWL kernel workflow.

This is intentionally not a generic workflow language. It is the first-stage
centralized state machine for workflow uid `kw`; all node implementations are
owned by this workflow under `workflow/kw/nodes/`.
"""
...
def transition(state: dict[str, Any], phase: str, result: NodeResult) -> str:
    ...
    if phase == "check_eval":
        ...
        state["resume_followups"] = [
            "Produce or repair a smooth full evaluation before exiting again.",
            "Run plan_eval.py --assess with canonical baseline frames.",
            "Record or backfill the candidate once assess_verdict.json exists.",
        ]
        return "write_resume"
    ...
    if phase == "reviewer":
        if result.outcome == "accepted":
            state["status"] = "done"
            state["terminal_reason"] = state.get("reviewer_reason") or "reviewer_accepted"
            return "done"
        ...
        state["resume_followups"] = [
            "Write REVIEWER-STATUS.json with status=accepted or status=needs_executor_resume."
        ]
        return "write_resume"
...
def run_loop(args: argparse.Namespace) -> dict[str, Any]:
    ...
    goal_default = Path(meta["goal_dir"]) if meta.get("goal_dir") else worktree / "goals" / "kwl-fusion"
    goal_dir = resolve_path(worktree, args.goal_dir, goal_default)
    state_path = resolve_path(worktree, args.state_file, worktree / "state" / "workflow-kw-state.json")
    event_log = resolve_path(worktree, args.event_log, worktree / "state" / "workflow-kw-events.jsonl")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--experiment-uid", help="UID like hunyuan-kw-0001")

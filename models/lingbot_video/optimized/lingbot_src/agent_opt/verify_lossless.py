"""Lossless gate: compare a config's refined video against the golden reference.
...
Usage:
  python agent_opt/verify_lossless.py <golden.mp4> <config.mp4> [--min-psnr 45]
...
def main():
    ...
    try:
        ...
        c = read_frames(args.config)
    except Exception as e:  # noqa: BLE001
        print(f"LOSSLESS_VERDICT ERROR reason=read_failed detail={e!r}")
        return 2

    if len(g) != len(c):
        print(f"LOSSLESS_VERDICT FAIL reason=frame_count golden={len(g)} config={len(c)}")
        return 1
    if not g:
        print("LOSSLESS_VERDICT ERROR reason=empty")
        return 2
    if g[0].shape != c[0].shape:
        print(f"LOSSLESS_VERDICT FAIL reason=shape golden={g[0].shape} config={c[0].shape}")
        return 1

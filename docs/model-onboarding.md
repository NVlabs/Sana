...
profile, then express each config's required capabilities in its manifest.

## Current Contract

1. `models/<id>.toml` records official config, runtime env, baseline metadata,
   run script, and human seam/status notes.
2. Config TOML files declare `[requirements].capabilities`.
3. `efficiency.config_manifest.dry_run_manifest()` synthesizes a minimal
...
3. For each config, implement the pure algorithm in `efficiency/` when it is
   model-agnostic, and keep model-specific consumption in `Sol-LTX-Infer/`.
4. Declare only the capabilities that config needs in its TOML.

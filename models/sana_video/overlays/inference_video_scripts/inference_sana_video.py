...
from diffusion import (
    DPMS,
    AutoregressiveChunkFlowEuler,
    ChunkFlowEuler,
    FlowEuler,
    LongLiveFlowEuler,
    LongLiveFlowEulerRolling,
    LTXFlowEuler,
    SelfForcingFlowEuler,
)
from diffusion.data.datasets.utils import *
from diffusion.data.transforms import read_image_from_path, read_video_from_path
...
from diffusion.model.registry import init_registry
...
from tools.download import find_model, resolve_model_path
...
def write_hot_timing(records, warmup_samples):
    output = os.environ.get("SANA_HOT_TIMING_OUTPUT", "").strip()
    if not output:
        raise RuntimeError("SANA_HOT_TIMING_OUTPUT is required when SANA_HOT_BENCHMARK=1")
    if not records:
        raise RuntimeError("hot benchmark produced no measured samples")

    stage_keys = ("text_encoder_s", "denoise_s", "vae_decode_s", "total_s")
    aggregate = {key: sum(float(item[key]) for item in records) for key in stage_keys}
    aggregate.update(
        {
            "sample_count": len(records),
            "sample_mean_s": aggregate["total_s"] / len(records),
            "sample_min_s": min(float(item["total_s"]) for item in records),
            "sample_max_s": max(float(item["total_s"]) for item in records),
            "sample_total_s": aggregate["total_s"],
        }
    )
    payload = {
        "schema_version": 1,
        "timing": {
            "scope": HOT_TIMING_SCOPE,
            "clock": "time.perf_counter_with_cuda_synchronize",
...
class DistributePromptsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            "key": key,
            "prompt": prompt,
            "global_idx": txt_line_idx,  # 使用txt文件中的原始行号
        }
...
def visualize(
    ...
    active_dataloader = dataloader if dataloader is not None else prompts_dataloader
    active_output_root = output_root if output_root is not None else save_root
    tqdm_desc = f"{active_output_root.split('/')[-1]} [{timing_mode}] GPU: {args.gpu_id}"
    for chunk in tqdm(active_dataloader, desc=tqdm_desc, unit="batch", position=rank, leave=True):
        # data prepare
        prompts, hw = (
            [],
        ...
        if bs == 1:
            ...
            if config.task == "ti2v" or config.task == "ltx" or config.task == "v2v":
        ...
        for i in range(bs):
            ...
            save_path = os.path.join(active_output_root, save_file_name)
        ...
        measure_hot = timing_mode == "measure"
        if measure_hot:
            cuda_sync()
        ...
        if measure_hot:
            cuda_sync()
            text_encoder_finished = time.perf_counter()
        ...
        with torch.no_grad():
            ...
            if config.task == "ti2v" or config.task == "ltx":
                ...
                image_vae_embeds = vae_encode(
                    config.vae.vae_type, vae, torch.stack(images, dim=0).to(vae_dtype), device=device
                )  # 1,C,T,H,W
                if cfg_scale > 1.0:
                    image_vae_embeds = torch.cat([image_vae_embeds, image_vae_embeds], dim=0)  # 2,C,1,H,W
            ...
            elif args.sampling_algo == "longlive_flow_euler_rolling":
                base_chunk_frames = base_model_frames // config.vae.vae_stride[0]
                flow_solver = LongLiveFlowEulerRolling(
                    model,
                    condition=caption_embs,
                    flow_shift=flow_shift,
                    model_kwargs=model_kwargs,
        ...
        if measure_hot:
            cuda_sync()
            denoise_finished = time.perf_counter()
        ...
        if measure_hot:
            ...
            if timing_records is None:
                raise RuntimeError("timing_records is required for measured hot inference")
            timing_records.append(
                {
                    "sample_index": int(chunk["global_idx"][0]),
                    "text_encoder_s": text_encoder_finished - sample_started,
                    "denoise_s": denoise_finished - text_encoder_finished,
                    "vae_decode_s": decode_finished - denoise_finished,
                    "total_s": decode_finished - sample_started,
                    "height": int(hw[0, 0]),
                    "width": int(hw[0, 1]),
        ...
        if config.task == "v2v":
            # import ipdb; ipdb.set_trace()
            concat_videos = []
            for i, video_prompt in enumerate(image_paths):
                video = videos[i]  # T,H,W,C
                source_video = read_video_from_path(
                    video_prompt, (video.shape[1], video.shape[2]), video.shape[0]
                )  # T,C,H,W, -1, 1
                source_video = (source_video + 1) * 127.5  # T,C,H,W, -1, 1 -> T,C,H,W, 0, 255
                concat_video = torch.cat(
                    [source_video.permute(0, 2, 3, 1).to(torch.uint8), video], dim=1
                )  # T,C,H,W -> T,H,W,C -> T,2H,W,C
                concat_videos.append(concat_video)
            concat_videos = torch.stack(concat_videos, dim=0)  # B,T,2H,W,C
            videos = concat_videos
        ...
        for i, video in enumerate(videos):
            ...
            writer.close()
            if args.save_qkv:
                qkv_save_path = os.path.join(active_output_root, f"qkv_{chunk['key'][i]}")
            ...
            if args.save_block_output:
                prompt_key_name = chunk["key"][i].replace(" ", "_")
                block_output_save_path = os.path.join(active_output_root, f"block_output_{prompt_key_name}")
...
class SanaInference(SanaVideoConfig):
    ...
    txt_file: str = "asset/samples/vbench/samples_vbench_long_extend.txt"
...
if __name__ == "__main__":
    ...
    if guidance_type == "adaptive_projected_guidance":
        ...
        apg = None
    # sample_steps_dict = {"dpm-solver": 50, "flow_dpm-solver": 50, "flow_euler": 50, "flow_euler_ltx": 50}
    ...
    if (
        config.task == "ti2v"
        and config.get("image_encoder") is not None
        and config.image_encoder.get("image_encoder_name") == "flux-siglip"
    ):
    ...
    # load attn mask
    if config.model.diagonal_mask:
        flex_attn_padding = 128
        diagonal_mask_path = f"{config.model.diagonal_mask_root}/nlogn_mask_h{int(latent_size_h/model.patch_size[-2])}_w{int(latent_size_w/model.patch_size[-1])}_f{int(latent_size_t/model.patch_size[0])}_p{flex_attn_padding}_b{config.model.diagonal_block_size}.pt"
        logger.info(f"Loading diagonal mask from {diagonal_mask_path}")
        model.load_diagonal_mask(path=diagonal_mask_path)

    resolved_model_path = resolve_model_path(args.model_path, use_ema=args.use_ema)
    logger.info(f"Generating sample from ckpt: {args.model_path}")
    logger.info(f"Resolved checkpoint path: {resolved_model_path}")
    state_dict = find_model(resolved_model_path)
    if resolved_model_path.endswith(".bin"):
        logger.info("Loading fsdp bin checkpoint....")
        state_dict = {"state_dict": state_dict}

    if "generator" in state_dict:  # used for loading Longlive checkpoints
        state_dict = state_dict["generator"]
    if "state_dict_ema" in state_dict and "state_dict" not in state_dict:
        state_dict = {"state_dict": state_dict["state_dict_ema"]}
    elif "state_dict" not in state_dict:
        new_state_dict = dict()
    ...
    if args.work_dir is None:
        ...
        if "checkpoints" in path_parts:
            ...
            work_dir_parts = path_parts[:ckpt_idx]
        else:
            work_dir_parts = path_parts[:-2]
        work_dir = (
            f"/{os.path.join(*work_dir_parts)}" if args.model_path.startswith("/") else os.path.join(*work_dir_parts)
        )
    else:
    ...
    if args.save_block_output:
        ...
        model.register_block_hook(device="cpu", score_only="l2")
    ...
    hot_benchmark = env_flag("SANA_HOT_BENCHMARK", "0")
    warmup_samples = int(os.environ.get("SANA_HOT_WARMUP_SAMPLES", "1")) if hot_benchmark else 0
    warmup_samples = min(max(warmup_samples, 0), len(items))
    warmup_dataloader = None
    if warmup_samples:
        warmup_dataset = DistributePromptsDataset(items[:warmup_samples], original_indices[:warmup_samples])
        warmup_dataloader = torch.utils.data.DataLoader(warmup_dataset, batch_size=args.bs, shuffle=False)
        warmup_dataloader = accelerator.prepare(warmup_dataloader)
    if num_frames > base_model_frames:
        assert args.sampling_algo in [
            "autoregressive_flow_euler",
            "self_forcing_flow_euler",
            "longlive_flow_euler",
            "longlive_flow_euler_rolling",
        ]
    ...
    def create_save_root(args, dataset, epoch_name, step_name, sample_steps, num_frames):
        save_root = os.path.join(
            ...
            f"_step{sample_steps}_size{args.image_size}_f{num_frames}_bs{args.bs}_samp{args.sampling_algo}"
        ...
        if args.high_motion and not config.task == "v2v":
            save_root += f"_highmotion"
        if args.motion_score > 0 and not config.task == "v2v":
            save_root += f"_motion{args.motion_score}"
        if args.negative_prompt != "":
            save_root += f"_negp{args.negative_prompt[:5].replace(' ', '')}"
        if args.num_recache > 0:
            save_root += f"_base{base_model_frames}_numrecache{args.num_recache}"
    ...
    elif args.txt_file is not None and "hunyuan" in args.txt_file:
        dataset = "hunyuan"
        if "i2v" in args.txt_file:
            dataset += "_i2v"
        else:
            dataset += "_t2v"
    elif args.txt_file is not None and "vbench" in args.txt_file:
        dataset = "vbench"
    else:
        dataset = args.dataset
    if args.ablation_selections and args.ablation_key:
        ...
        if warmup_dataloader is not None:
            logger.info(f"Running {warmup_samples} unmeasured same-shape hot benchmark warmup sample(s)")
            visualize(
                config=config,
                args=args,
                model=model,
                items=items[:warmup_samples],
                bs=args.bs,
                sample_steps=sample_steps,
                cfg_scale=args.cfg_scale,
        ...
        visualize(
            ...
            sample_steps=sample_steps,
            cfg_scale=args.cfg_scale,
            timing_mode="measure" if hot_benchmark else "off",
            timing_records=hot_timing_records,
        )
        if hot_benchmark and accelerator.is_main_process:
            write_hot_timing(hot_timing_records, warmup_samples)

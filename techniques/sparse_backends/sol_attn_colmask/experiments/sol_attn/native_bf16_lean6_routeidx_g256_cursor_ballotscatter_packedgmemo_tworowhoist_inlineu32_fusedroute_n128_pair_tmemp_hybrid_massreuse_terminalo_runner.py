"""Runner for the isolated G256 B9 scheduling/lifecycle parity candidate."""
...
KERNEL_PATH = ROOT / (
    ...
    "native_bf16_lean6_routeidx_g256_cursor_ballotscatter_packedgmemo_tworowhoist_inlineu32_fusedroute_n128_pair_tmemp_hybrid_"
    "massreuse_terminalo_fwd.py"
)
KERNEL_SHA256 = "ffc9c49f97963707b332f1a939f6fafe886e423a9ddad4c19eb2c4e93cf0aeee"
...
def _validate_kernel_identity() -> str:
    ...
    if observed != KERNEL_SHA256:
        raise RuntimeError(
            f"G256-B9-parity/N128 kernel SHA mismatch: {observed} != "
            f"{KERNEL_SHA256}"
        )
    return observed
...
def make_native_bf16_lean6_routeidx_g256_cursor_ballotscatter_packedgmemo_tworowhoist_inlineu32_fusedroute_n128_pair_tmemp_hybrid_massreuse_terminalo_runner(
    T: int,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    *,
    ...
    from kernels.sol_attn_sm100.native_bf16_lean6_routeidx_g256_cursor_ballotscatter_packedgmemo_tworowhoist_inlineu32_fusedroute_n128_pair_tmemp_hybrid_massreuse_terminalo_fwd import (
    ...
    op = (
        build_sol_attn_sm100_lean6_routeidx_g256_cursor_ballotscatter_packedgmemo_tworowhoist_inlineu32_fusedroute_n128_pair_tmemp_bf16_trace_fwd(T)
        if trace_route_masks
        else build_sol_attn_sm100_lean6_routeidx_g256_cursor_ballotscatter_packedgmemo_tworowhoist_inlineu32_fusedroute_n128_pair_tmemp_bf16_fwd(T)
    )
    output_storage = None
    lse_storage = None
    if guard_elements:
    ...
    run.b9_donor_kernel_source_sha256 = (
    ...
    run.ballot_scatter = True
    run.route_index_writer = "selected_lane_rank_addressed_scatter"
    run.route_packet_writer = "owner_warp0_lane0"
    run.new_named_or_cta_barrier = False
    run.terminal_o_transport = (
        "two_row_invariant_hoisted_bf16x2_inline_st_global_u32"
    )
    run.terminal_o_stores_per_owner_thread = 32
    run.terminal_o_stats_loads_per_owner_thread = 2
    run.terminal_o_reciprocals_per_owner_thread = 2
    ...
    run.collect_both_score_halves_before_execute = False
    run.collect_both_index_halves_before_exact = True
...
__all__ = [
    "KERNEL_SHA256",
    "make_native_bf16_lean6_routeidx_g256_cursor_ballotscatter_packedgmemo_tworowhoist_inlineu32_fusedroute_n128_pair_tmemp_hybrid_massreuse_terminalo_runner",
]

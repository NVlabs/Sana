...
def gemm_ptx_precomputed_varname(
    ...
    if const_expr(not is_ts):
        llvm.inline_asm(
            ...
            ".reg .b32 smem_desc_a_lo, smem_desc_b_lo;\n\t"
            ".reg .b32 smem_desc_a_hi, smem_desc_b_hi;\n\t"
            # The dense path intentionally chains q0=-stride then q1=+stride.
            # Sparse q-stage2 issues either side independently and therefore
            # requests immutable derived descriptors instead.
            + (
                f".reg .b64 smem_desc_a_<{num_k_tile}>;\n\t"
                if not mutate_smem_desc
                else ""
            )
            ...
            + (
                "mov.b64 smem_desc_a_0, {smem_desc_a_lo, smem_desc_a_hi};\n\t"
                if not mutate_smem_desc
                else f"mov.b64 {smem_var_name_prefix}_0, {{smem_desc_a_lo, smem_desc_a_hi}};\n\t"
            )
            ...
            + "".join(
                (
                    ...
                    f"add.s32 smem_desc_b_lo, smem_desc_b_lo_start, {hex(offset_b[k])};\n\t"
                    + (
                        f"mov.b64 smem_desc_a_{k}, {{smem_desc_a_lo, smem_desc_a_hi}};\n\t"
                        if not mutate_smem_desc
                        else f"mov.b64 {smem_var_name_prefix}_{k}, {{smem_desc_a_lo, smem_desc_a_hi}};\n\t"
                    )
            ...
            + f"@leader_thread tcgen05.mma.cta_group::{cta_group}.kind::{kind} [tmem_acc], {('smem_desc_a_0' if not mutate_smem_desc else f'{smem_var_name_prefix}_0')}, smem_desc_b_0, {idesc_var_name}, {pred_str};\n\t"
            + "".join(
                (
                    # f"mov.b64 {{smem_desc_a_lo, smem_desc_a_hi}}, {smem_var_name_prefix}_{k};\n\t"
                    # f"add.s32 smem_desc_a_lo, smem_desc_a_lo, {smem_offset};\n\t"
                    ...
                    f"@leader_thread tcgen05.mma.cta_group::{cta_group}.kind::{kind} [tmem_acc], {('smem_desc_a_' + str(k) if not mutate_smem_desc else f'{smem_var_name_prefix}_{k}')}, smem_desc_b_{k}, {idesc_var_name}, 1;\n\t"
                )
                for k in range(1, num_k_tile)
            )
            + "}\n",

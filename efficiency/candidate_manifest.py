"""Candidate-manifest validation and model-agnostic dry-run helpers."""
...
def manifest_id(data: dict[str, Any]) -> str:
    ...
    if isinstance(raw, dict):
        value = raw.get("name") or raw.get("candidate") or raw.get("id")
        if value:
            return str(value)
    if raw is not None:
        return str(raw)
...
def is_efficiency_candidate(data: dict[str, Any]) -> bool:
...
def schema_errors(data: dict[str, Any], repo_root: Path) -> list[str]:
    if not is_efficiency_candidate(data):
        return []

    errors: list[str] = []
    raw_id = data.get("id")
    if not isinstance(raw_id, dict):
        errors.append("[id] table is required for efficiency candidates")
    else:
...
def dry_run_manifest(
    ...
    *,
    ...
    if errors:
        raise ValueError("candidate schema errors:\n  - " + "\n  - ".join(errors))

    from efficiency import compose
    from efficiency.transform import ModelTransform

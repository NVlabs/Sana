...
Append one entry per config. Newest at the bottom. The frontier is the set of
...
- Hypothesis: before spending budget on the attention bottleneck, measure where the 237s
  ...
  LINGBOT_PHASE_TIMING=1 → lossless no-op path). Config = golden config + timing.
...
- Node variance ~7%; compare config on PHASE deltas, not just raw wall clock.

"""Codegen diagnostic: logical-G512 fused route with uniform outer cursors.

This file is an isolated lowering experiment over the frozen G512 fused-route
kernel. It preserves every route/exact phase and mathematical operation, but
expresses the runtime outer-loop induction explicitly and marks both carried
cursors warp-uniform. There is no shape, density, or dispatch fast path.
...
# This child keeps one lowbit chain per physical packet, appends halves 0..3
# in least-significant-bit order, and therefore preserves ascending block
# order exactly.  The 2 KiB G512 index stream is resident only in SMEM;
# routing indices never touch HBM.
...
ROLE_MAP = {
    ...
    "warp1_4": "logical_g512_append_selector_then_combined_exact_stream",
    "warp5": "independent_single_stage_n128_k_v_tma_producer",
}
MECHANISM_FAMILY = (
    "sm100-bf16-logical-g512-cursor-codegen-diagnostic-v1"
)
CANDIDATE_AXIS = (
    "uniform_outer_loop_induction_only_no_phase_or_math_change"
)
...
G512_CURSOR_FUSEDROUTE_N128_TMEMP_RECEIPT = {
    "codegen_parent_source_sha256": CODEGEN_PARENT_SOURCE_SHA256,
    "diagnostic_axis": "uniform_outer_loop_induction_only",
    ...
    "route_index_happens_before": {
        "single_writer": "owner_warp0_lane0_appends_halves0_to3",
        "half_append_order": "same_thread_program_order_plus_cumulative_packet_word6",
        "publish_to_warp5": "fence_view_async_shared_then_full_cta_preexact_join",
...
class SharedStorage:
    ...
    # Owner-warp 0 lane 0 appends all present N128 route masks.  The full-CTA
...
@cute.kernel
def lean6_bf16_routeidx_g512_cursor_fusedroute_n128_pair_tmemp_grid_kernel(
                    ...
                    for word in cutlass.range_constexpr(ROUTE_MASK_WORDS):
                        ...
                        if valid:
                            ...
                            if pisa2_route_is_exact(
                                ...
                                bit_value = Int32(1) << lane
                        bit_value = bit_value | cute.arch.shuffle_sync_down(
                            bit_value, 16
                        )
                        bit_value = bit_value | cute.arch.shuffle_sync_down(
                            bit_value, 8
                        ...
                        if lane == Int32(0):
                            if cutlass.const_expr(word == 0):
                                ...
                                mask3 = bit_value
                    if lane == Int32(0):
                        exact_count = pisa2_count_exact_bits_popc(
                            mask0, mask1, mask2, mask3
                        )
                        # Half 0 starts a fresh G512 stream; halves 1..3 append
                        # to the cumulative count published by the same writer.
                        # No consumer reads the list until the one full-CTA
                        # pre-exact join after the inner route loop.
                        append_base = Int32(0)
                        if half_idx != Int32(0):
                            append_base = Int32(route_packet[6])
                        route_rank = append_base
                        for word in cutlass.range_constexpr(ROUTE_MASK_WORDS):
                            m = mask0
                            if cutlass.const_expr(word == 1):
                                m = mask1
                            elif cutlass.const_expr(word == 2):
                                m = mask2
                            elif cutlass.const_expr(word == 3):
                                m = mask3
                            while m != Int32(0):
                                lowbit = m & (Int32(0) - m)
                                bit = pisa2_bfind_b32(lowbit)
                                route_indices[route_rank] = (
                                    route_start + Int32(word * 32) + bit
                                )
                                route_rank = route_rank + Int32(1)
                                m = m & (m - Int32(1))
...
@cute.jit
def lean6_bf16_routeidx_g512_cursor_fusedroute_n128_pair_tmemp_grid_host(
    q: cute.Tensor,
    k: cute.Tensor,
    v: cute.Tensor,
    o: cute.Tensor,
    ...
    lean6_bf16_routeidx_g512_cursor_fusedroute_n128_pair_tmemp_grid_kernel(
        tiled_pack_qk,
        tiled_pack_pv,
        q_tma_atom,
        q_tma_tensor,
...
class Pisa2Sm100Lean6RouteidxG512CursorFusedrouteN128PairTmempBf16Forward:
    ...
    def __call__(
        ...
        return lean6_bf16_routeidx_g512_cursor_fusedroute_n128_pair_tmemp_grid_host(
            q,
            k,
            v,
            o,
...
def build_pisa2_sm100_lean6_routeidx_g512_cursor_fusedroute_n128_pair_tmemp_bf16_fwd(
    T: int,
    *,
    is_causal: bool = False,
) -> Pisa2Sm100Lean6RouteidxG512CursorFusedrouteN128PairTmempBf16Forward:
    """Build the ordinary no-trace full-grid sibling."""

    return Pisa2Sm100Lean6RouteidxG512CursorFusedrouteN128PairTmempBf16Forward(
        T, is_causal=is_causal
    )


def build_pisa2_sm100_lean6_routeidx_g512_cursor_fusedroute_n128_pair_tmemp_bf16_trace_fwd(
    T: int,
    *,
    is_causal: bool = False,
) -> Pisa2Sm100Lean6RouteidxG512CursorFusedrouteN128PairTmempBf16Forward:
...
__all__ = [
    ...
    "ROUTE_TILE_SIZE",
    "UNCOMPENSATED_V9_SOURCE_SHA256",
    "build_pisa2_sm100_lean6_routeidx_g512_cursor_fusedroute_n128_pair_tmemp_bf16_fwd",
    "build_pisa2_sm100_lean6_routeidx_g512_cursor_fusedroute_n128_pair_tmemp_bf16_trace_fwd",
]

...
# already appears in config filenames and in each SOURCE_SNAPSHOT's "variant"
...
def _config(hardware: str, profile: str) -> dict:
    path = CONFIG / "minimax_h3" / f"{hardware}_{profile}.toml"
...
def test_config_point_directly_to_one_hardware_runtime() -> None:
    ...
    for hardware in RUNTIMES:
        ...
        for profile in profiles:
            config = _config(hardware, profile)
            assert config["submodule"] == expected_root
            assert config["runtime"]["root"] == expected_root
            assert "H3_HARDWARE" not in config["env"]
            assert config["env"]["H3_SOL_PROFILE"] == profile
            assert config["env"]["H3_CONTAINER_IMAGE"] == PINNED_IMAGE
            assert not config["inherit_profile_env"]
            assert config["official_config"]["num_gpus"] == 4
            assert config["official_config"]["context_parallel_degree"] == 4
...
def test_config_do_not_inherit_the_legacy_diffusers_environment() -> None:
    namespace = runpy.run_path(str(ROOT / "scripts/launch_config.py"))
    merge = namespace["merge_model_profile"]
    for hardware in RUNTIMES:
        merged = merge(_config(hardware, "fullopt_exact"))

...
# Vendored from cuDNN BSA v126; only helper imports are redirected to the
# equivalent private FlashAttention/CuTe source closure in this repository.
import math
...
class BlockSparseAttnForwardCombine:
    def __init__(
        ...
        num_threads: int = 256,
        stages: int = 4,
        partial_dtype: Type[cutlass.Numeric] = Float32,
    ):
        """
        ...
        """
        self.dtype = dtype
        if partial_dtype not in (Float32, cutlass.BFloat16):
            raise TypeError("O partial dtype must be Float32 or BFloat16")
    ...
    def __call__(
        ...
        if const_expr(not (mO_partial.element_type == self.partial_dtype)):
            raise TypeError(
                "O partial tensor must match the configured partial dtype"
            )

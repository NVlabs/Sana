...
# run_ltx23_sglang_hq pipeline + piecewise_attn backend already consume.
...
class SparseAttention(ModelTransform):
    ...
    is the per-step schedule's warmup (first-N dense) expressed as an int for the
    existing kernel; ``component`` selects which transformer the backend applies
    to (e.g. "transformer_2" = stage-2 only, the full-opt setting)."""

...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def load_base_model_peft_ckpt(self, pretrained_model_path, init_patch_embedding=False, verbose=True):
        """
        Load base model weights into a PEFT-wrapped model.
        This function specifically handles PEFT models and maps base model parameters
        to the correct nested structure including base_layer for LoRA-wrapped parameters.
        NOTE: here, it is not wrapped by PeftModel, it is PeftModel.base_model, so the key do not contain base_model.
        If we load a pre-trained lora model, outside of this function, we just use self.load_state_dict(pretrained_model_state_dict)

        Args:
            pretrained_model_path: Path to checkpoint containing base model weights
            init_patch_embedding: Whether to initialize patch embedding weights
            verbose: Whether to print detailed logging
        """
        logger = get_logger(__name__)
        logger.info(f"======> Loading base model weights from {pretrained_model_path} into PEFT model <=======")
        ...
        # Get current PEFT model state dicts
        full_state_dict = self.state_dict()  # Complete PEFT model state dict

        logger.info(f"PEFT model structure: {len(full_state_dict)} total parameters")

        new_state_dict = {}
        non_matched_keys = []
        matched_count = 0

        # Create a mapping from base model keys to PEFT structure keys
        peft_key_mapping = {}
        for peft_key in full_state_dict.keys():
            # Extract the base model key
            if ".base_layer." in peft_key:
                # This is a LoRA-wrapped parameter: base_model.model.blocks.X.layer.base_layer.weight
                # Extract the base model key: blocks.X.layer.weight
                base_key = peft_key.replace(".base_layer.", ".")
            else:
                # Regular parameter: base_model.model.layer.weight -> layer.weight
                base_key = peft_key

            peft_key_mapping[base_key] = peft_key

        logger.info(f"Found {len(peft_key_mapping)} base model parameters that map to PEFT structure")

        # Initialize all current PEFT parameters
        for k, v in full_state_dict.items():
            new_state_dict[k] = v

        # Load base model parameters into correct PEFT structure
        for base_key, peft_key in peft_key_mapping.items():
            if base_key in pretrained_model_state_dict:
                pretrained_value = pretrained_model_state_dict[base_key]

                # Check shape compatibility
                if full_state_dict[peft_key].shape == pretrained_value.shape:
                    # Find the correct PEFT key for this base model parameter
                    new_state_dict[peft_key] = pretrained_value
                    matched_count += 1
                else:
                    logger.warning(
                        f"Shape mismatch for {base_key}: current {full_state_dict[peft_key].shape} vs pretrained {pretrained_value.shape}"
                    )
                    non_matched_keys.append(base_key)
            else:
                non_matched_keys.append(base_key)

        # Handle patch embedding specially - if non-matched, keep current weights
        if "patch_embedding.weight" in non_matched_keys:
            # if weight is not matched, bias should also not be matched
            non_matched_keys.append("patch_embedding.bias")
            peft_key = peft_key_mapping["patch_embedding.bias"]
            matched_count -= 1
            new_state_dict[peft_key] = full_state_dict[peft_key]

        if init_patch_embedding:
            logger.info("======> Force initializing patch embedding and head weights <=======")
            # Keep current weights for these 4 parameters regardless of whether they're in pretrained
            init_keys = ["patch_embedding.weight", "patch_embedding.bias", "head.head.weight", "head.head.bias"]
            for init_key in init_keys:
                if init_key in peft_key_mapping:
                    peft_key = peft_key_mapping[init_key]
                    new_state_dict[peft_key] = full_state_dict[peft_key]
                    logger.info(f"Keeping current {init_key} as initialized (PEFT key: {peft_key})")
                else:
                    # For non-PEFT keys, use the key directly
                    if init_key in full_state_dict:
                        new_state_dict[init_key] = full_state_dict[init_key]
                        logger.info(f"Keeping current {init_key} as initialized")

                # Ensure these keys are marked as non-matched
                if init_key not in non_matched_keys:
                    non_matched_keys.append(init_key)

                # If we had matched this key before, decrement the count
                if init_key in peft_key_mapping and init_key in pretrained_model_state_dict:
                    matched_count -= 1

        # Log results
        if verbose:
            logger.info(f"Parameter loading summary:")
            logger.info(f"  📊 Total model parameters: {len(full_state_dict)}")
            logger.info(f"  ✅ Successfully loaded from pretrained: {matched_count}")
            logger.info(f"  ⚠️  Kept current weights (non-matched): {len(non_matched_keys)}")

            if non_matched_keys and verbose:
                logger.info("Parameters using current weights:")

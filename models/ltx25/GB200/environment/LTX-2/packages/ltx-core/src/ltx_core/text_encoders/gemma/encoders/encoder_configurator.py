...
def _load_gemma_config(gemma_model_path: str) -> object:
    """Load the Hugging Face config from a Gemma model directory."""
    config_dir = find_matching_file(gemma_model_path, "config.json").parent
...
def _parse_version(version: str) -> tuple[int, ...]:
    """Parse the leading numeric dotted components, ignoring any non-numeric suffix.
    Checkpoint ``model_version`` strings aren't always plain dotted integers (e.g. the pre-2.4
    era used pre-release tags like ``"2.3.rc1"``). Only the numeric prefix is meaningful for the
    ``>= 2.4.0`` comparison, and a shorter numeric prefix already compares as "less than" a
    longer one with the same leading components (``(2, 3) < (2, 4, 0)``), so dropping the
    non-numeric tail is safe.
    """
    numeric_parts = []
...
def _check_gemma_version(metadata: dict, gemma_config: object, gemma_model_path: str) -> None:
    """Verify the checkpoint's expected Gemma version matches the Gemma model being loaded.
    Checkpoints that declare a ``gemma_source_checkpoint`` (LTX 2.4+) must match its
    ``gemma_version`` against the Gemma config's top-level ``gemma_version``. Older
    checkpoints (no ``gemma_source_checkpoint``) are only checked for using a Gemma 3
    text encoder, unless their declared ``model_version`` is already >= 2.4.0, in which
    case a ``gemma_source_checkpoint`` was required and its absence is itself an error.
    """
    ...
    if model_version is not None and _parse_version(model_version) >= _GEMMA_SOURCE_CHECKPOINT_REQUIRED_SINCE:
        actual_gemma_version = getattr(gemma_config, "gemma_version", None)
        if actual_gemma_version is None:
            logger.warning(
                "Checkpoint model_version=%r is >= 2.4.0 and does not declare "
                "gemma_source_checkpoint.gemma_version, and the Gemma config at %r also has no "
                "gemma_version. Skipping version check — verify compatibility manually.",
                model_version,
                gemma_model_path,
            )
            return
        raise ValueError(
            f"Checkpoint model_version={model_version!r} is >= 2.4.0 and must declare "
...
def _build_gemma4_unified_llm_key_ops() -> SDOps:
    """Checkpoint-key remapping for Gemma 4 unified (e.g. gemma-4-12B, model_type gemma4_unified).
    Every key sits under a single model. prefix, so we prepend a second model. to land each
    component at model.model.<component>.X. Two names differ from the loaded transformers module
    tree: the vision patch embedder ships as vision_embedder. but loads as embed_vision., and its
    projection ships as embed_vision.embedding_projection. but loads under
    embed_vision.multimodal_embedder.. The audio embedder maps straight through.
    """
    return _with_tied_lm_head(
        SDOps("GEMMA4_UNIFIED_LLM_KEY_OPS")
...
def get_gemma_ops(gemma_model_path: str) -> tuple[SDOps, tuple[ModuleOps, ...]]:
    ...
    if model_type == "gemma4_unified":
        ...
        return _build_gemma4_unified_llm_key_ops(), (gemma4_unified_model_ops, *asset_ops)

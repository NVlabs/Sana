#!/usr/bin/env python3
"""Freeze Triton BF16 SOL Attention semantics and gate a CuTe BF16 candidate.
...
CONTRACT = "sol_attn_bf16_triton_bitwise_oracle_cutedsl_gate_v1"
INPUT_CONTRACT = "sol_attn_bf16_frozen_qkv_v1"
BLOCK_SIZE = 64
HEAD_DIM = 128
GROUP_SIZE = 64
...
def _independent_references(torch: Any, prepared_reference: Any, case: CaseSpec, q: Any, k: Any, v: Any, kc: Any, vc: Any, q_scale: Any, k_scale: Any, threshold: Any, scale: float) -> Tuple[Any, Any, Dict[str, Any]]:
    ...
    evidence = {"kind": "prepared_sol_attn_fp32_model"}
    if case.threshold_mode == "all_exact":
...
def main() -> None:
    ...
    import torch
    import triton
    from experiments.sol_attn.prepared_reference import sol_attn_prepared_reference
    ...
    with args.output.open("x", encoding="utf-8") as handle:
        ...
        for case in cases:
            try:
                with torch.no_grad():
                    row = _run_case(
                        torch, triton, aligned, sol_attn_prepared_reference,

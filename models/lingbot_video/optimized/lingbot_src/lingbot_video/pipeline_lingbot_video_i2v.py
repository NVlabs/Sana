...
class LingBotVideoImageToVideoPipeline(LingBotVideoPipeline):
    ...
    def __call__(
        ...
        # --- Optional step-EasyCache on the REFINER DiT (gated, separate params) ---
        # Same pattern as the base pipeline; the refiner is short (8 steps) so the
        # dense guards are tighter. Reuse decision is a pure function of replicated
        # `latents` (identical on all CP ranks). Not bit-exact; user quality-gated.
        import os as _os
        _rc_enable = (
            _os.environ.get("LINGBOT_EASYCACHE_REFINER_ENABLE", "0").strip().lower()
            in {"1", "true", "yes", "on"}
        )
        _rc_thresh = float(_os.environ.get("LINGBOT_EASYCACHE_REFINER_THRESHOLD", "0.08"))
        _rc_head = int(_os.environ.get("LINGBOT_EASYCACHE_REFINER_HEAD_STEPS", "2"))
        _rc_tail = int(_os.environ.get("LINGBOT_EASYCACHE_REFINER_TAIL_STEPS", "1"))
        _rc_max_reuse = int(_os.environ.get("LINGBOT_EASYCACHE_REFINER_MAX_REUSE", "2"))
        _rc_nsteps = int(self.scheduler.timesteps.shape[0])
        _rc = {"ref": None, "cache": None, "reuse": 0, "compute": 0, "reused": 0}
        ...
        for i, timestep in enumerate(self.progress_bar(self.scheduler.timesteps)):
            ...
            if cfg_parallel:
                ...
                if do_cfg and effective_batch_cfg:
                    ...
                    if (
                        _rc_enable
                        and _rc["cache"] is not None
                        and _rc_head <= i < (_rc_nsteps - _rc_tail)
                        and _rc["reuse"] < _rc_max_reuse
                    ):
                        _ref = _rc["ref"]
                        _rel = (latent_model_input - _ref).abs().mean() / (
                            _ref.abs().mean() + 1e-8
                        )
                        _rc_use = bool(float(_rel) < _rc_thresh)
                    if _rc_use:
                        noise_batched = _rc["cache"]
                        _rc["reuse"] += 1
                        _rc["reused"] += 1
                    else:
                        with _transformer_autocast(device, transformer_dtype):
                        ...
                        if _rc_enable:
                            _rc["cache"] = noise_batched
                            _rc["ref"] = latent_model_input.detach().clone()
                            _rc["reuse"] = 0
                            _rc["compute"] += 1
        ...
        if _rc_enable and ((not dist.is_initialized()) or dist.get_rank() == 0):
            print(
                f"[lingbot_refiner_easycache] steps={_rc_nsteps} compute={_rc['compute']} "
                f"reused={_rc['reused']} thresh={_rc_thresh} head={_rc_head} tail={_rc_tail}",
                flush=True,
            )

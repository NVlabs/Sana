...
def build_prompt(ctx: NodeContext) -> str:
    ...
    cache_scope = read_text(Path(__file__).with_name("cache_scope.md"))
    adaptive_search = read_text(Path(__file__).with_name("adaptive_search.md"))
    retention_policy = read_text(Path(__file__).with_name("retention_policy.md"))
...
def run(ctx: NodeContext) -> NodeResult:
    ...
    result = run_managed_codex(
        ...
        role="implementation",
        prompt=prompt,
        required_artifacts=["AGENT-STATUS.json", "CACHE-SEARCH-STATE.json"],
        output_path=out,
    )

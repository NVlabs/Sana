...
def test_create_model_experiment_copies_baseline_closure_only(tmp_path: Path) -> None:
    ...
    assert (worktree / "efficiency/config_manifest.py").exists()
    ...
    assert not (worktree / "config/kwl_fusion").exists()
    assert not (worktree / "config/step_cache").exists()
    ...
    assert "config/hunyuan_diffusers_baseline.toml" in manifest["copied_paths"]
    ...
    dry_run = subprocess.run(
        [
            ...
            "config/hunyuan_diffusers_baseline.toml",
            "--mode",
            "dry-run",
        ],
        cwd=worktree,

...
| Hardware | GPUs | Workload | End-to-end speedup | Full-opt transfeat |
|---|---:|---:|---:|---|
| GB200 | 8 | 1344x768 @ 5 s | **3.97x** | [`minimax_h3_fullopt.toml`](../../transfeat/minimax_h3/fullopt.toml) |
| GB10 (DGX Spark) | 1 | 832x480 @ 5 s | **3.92x** | [`minimax_h3_gb10_fullopt.toml`](../../transfeat/minimax_h3/gb10_fullopt.toml) |
| RTX 5090 | 1 | 1344x768 @ 5 s | **4.52x** | [`minimax_h3_rtx5090_fullopt.toml`](../../transfeat/minimax_h3/rtx5090_fullopt.toml) |
| H100 | 4 | 1344x768 @ 5 s | **3.56x** | [`minimax_h3_h100_fullopt_exact.toml`](../../transfeat/minimax_h3/h100_fullopt_exact.toml) |
| A100 | 4 | 1344x768 @ 5 s | **3.55x** | [`minimax_h3_a100_fullopt_exact.toml`](../../transfeat/minimax_h3/a100_fullopt_exact.toml) |
...
Run the launcher from the repository root and select the full-opt transfeat for your GPU from the
table above:

```bash
# Workstation
python scripts/launch_transfeat.py <transfeat> --mode local

# Slurm cluster
python scripts/launch_transfeat.py <transfeat> \
  --mode sbatch --confirm-submit
```

`local` executes the generated launch script directly. `sbatch` renders and submits a Slurm job;
without `--confirm-submit`, it only prepares the job bundle. The transfeat selects the matching
...
```bash
python scripts/launch_transfeat.py \
  transfeat/minimax_h3/h100_fullopt_exact.toml \
  --mode sbatch --confirm-submit \
...
Full-opt uses approximate caching and sparse attention; keep the released transfeat unchanged when

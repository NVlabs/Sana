...
from .transformer_lingbot_video import set_lingbot_pisa_step
...
class LingBotVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # --- Optional kernel fusion: torch.compile the DiT blocks (gated) ---
        # Compiles each transformer block once (fullgraph=False so the MoE
        # grouped_mm and refiner PISA triton kernels graph-break out). Inductor
        # fuses the modulation / norm / RoPE / dense-linear glue; with
        # LINGBOT_FUSED_QKV_LINEAR=1 the per-call qkv weight-cat also fuses away.
        import os as _kos
        _kc_nsteps = int(self.scheduler.timesteps.shape[0])
        # Compile only the BASE stage: the refiner runs PISA whose triton module is
        # importlib-loaded (not in sys.modules), which dynamo cannot resolve; the
        # base has no PISA and is the 40-step stage where compile pays off most.
        if (
            _kos.environ.get("LINGBOT_KERNEL_COMPILE") == "1"
            and _kc_nsteps >= int(_kos.environ.get("LINGBOT_KERNEL_COMPILE_MIN_STEPS", "20"))
            and not getattr(self.transformer, "_kernel_compiled", False)
        ):
            import torch as _kt
            _cm = _kos.environ.get("LINGBOT_COMPILE_MODE", "default")
            _n = 0
            for _blk in getattr(self.transformer, "blocks", []):
                _blk.forward = _kt.compile(_blk.forward, mode=_cm, fullgraph=False, dynamic=False)
                _n += 1
            self.transformer._kernel_compiled = True
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                print(f"[lingbot_kernel_compile] compiled {_n} blocks mode={_cm}", flush=True)
        # --- Optional step-level EasyCache on the base DiT (gated) ---
        # Reuses the batched transformer output across steps whose input latents
        # barely changed. Applied only to long (base) schedules; the 8-step
        # refiner is never cached. The reuse decision is a pure function of the
        # replicated `latents`, so it is identical on every CP rank (no collective
        # divergence). Not bit-exact: same-seed approximation, user quality-gated.
        import os as _os
        def _ecb(_n):
            return _os.environ.get(_n, "0").strip().lower() in {"1", "true", "yes", "on"}
        _ec_nsteps = int(self.scheduler.timesteps.shape[0])
        # Both the base t2v stage AND the refiner run through THIS pipeline (the
        # refiner uses LingBotVideoPipeline with a cond_latent, mode=refiner). Pick
        # the parameter set by schedule length: long => base, short => refiner.
        if _ec_nsteps >= int(_os.environ.get("LINGBOT_EASYCACHE_MIN_STEPS", "20")):
            _ec_tag = "base"
            _ec_enable = _ecb("LINGBOT_EASYCACHE_ENABLE")
            _ec_thresh = float(_os.environ.get("LINGBOT_EASYCACHE_THRESHOLD", "0.08"))
            _ec_head = int(_os.environ.get("LINGBOT_EASYCACHE_HEAD_STEPS", "4"))
            _ec_tail = int(_os.environ.get("LINGBOT_EASYCACHE_TAIL_STEPS", "2"))
            _ec_max_reuse = int(_os.environ.get("LINGBOT_EASYCACHE_MAX_REUSE", "2"))
        else:
            _ec_tag = "refiner"
            _ec_enable = _ecb("LINGBOT_EASYCACHE_REFINER_ENABLE")
            _ec_thresh = float(_os.environ.get("LINGBOT_EASYCACHE_REFINER_THRESHOLD", "0.08"))
            _ec_head = int(_os.environ.get("LINGBOT_EASYCACHE_REFINER_HEAD_STEPS", "2"))
            _ec_tail = int(_os.environ.get("LINGBOT_EASYCACHE_REFINER_TAIL_STEPS", "1"))
            _ec_max_reuse = int(_os.environ.get("LINGBOT_EASYCACHE_REFINER_MAX_REUSE", "2"))
        _ec = {"ref": None, "cache": None, "reuse": 0, "compute": 0, "reused": 0}
        for i, timestep in enumerate(self.progress_bar(self.scheduler.timesteps)):
            set_lingbot_pisa_step(i, int(self.scheduler.timesteps.shape[0]))
            if cfg_parallel:
            ...
            if cfg_parallel:
                ...
                if do_cfg and effective_batch_cfg:
                    ...
                    if (
                        _ec_enable
                        and _ec["cache"] is not None
                        and _ec_head <= i < (_ec_nsteps - _ec_tail)
                        and _ec["reuse"] < _ec_max_reuse
                    ):
                        _ref = _ec["ref"]
                        _rel = (latent_model_input - _ref).abs().mean() / (
                            _ref.abs().mean() + 1e-8
                        )
                        _ec_use = bool(float(_rel) < _ec_thresh)
                    if _ec_use:
                        noise_batched = _ec["cache"]
                        _ec["reuse"] += 1
                        _ec["reused"] += 1
                    else:
                        with _transformer_autocast(device, transformer_dtype):
                        ...
                        if _ec_enable:
                            _ec["cache"] = noise_batched
                            _ec["ref"] = latent_model_input.detach().clone()
                            _ec["reuse"] = 0
                            _ec["compute"] += 1
        ...
        if _ec_enable and ((not dist.is_initialized()) or dist.get_rank() == 0):
            print(
                f"[lingbot_easycache:{_ec_tag}] steps={_ec_nsteps} compute={_ec['compute']} "
                f"reused={_ec['reused']} thresh={_ec_thresh} head={_ec_head} tail={_ec_tail}",
                flush=True,
            )

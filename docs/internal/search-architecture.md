...
model-specific config directly. The `efficiency` engine and `ModelSpec`
...
3. The subagent runs the bounded dimension loop: observe current-experiment
   results, propose a new hypothesis, implement exactly one config,
...
4. **(GPU stage)** Render a run bundle from the profile + cfg → existing
   `scripts/launch_config.py` → `scripts/collect_run.py` → `benchmark.json`/
...
## Quality eval pipeline (per config) — what ranks a config

Each search-loop iteration runs a config and assesses quality through THREE
...
whether a config is retained during fan-out and how it can be selected later.
During the per-dimension loop, quality is not a hard per-tier retention gate:
retain config that improve quality or speed/memory, and discard config
...
best-quality retained frontier config for each speed target:
...
## Loop control and failed config

A native goal dimension is not complete when one config fails. Failed
config must be rejected or discarded, logged with a failure signature or
reason, and used to choose the next hypothesis. Retained config update the
...
eval stage itself (real config runs) is GPU-side — `search.plan_eval` (stub).

#!/usr/bin/env python3
"""Workflow-local Codex executor for PISA attention optimization."""
...
def build_prompt(ctx: NodeContext) -> str:
    ...
    attention_scope = read_text(Path(__file__).with_name("attention_scope.md"))
    pisa_search = read_text(Path(__file__).with_name("pisa_search.md"))
    recipe_contract = read_text(Path(__file__).with_name("recipe_contract.md"))
    decision_policy = read_text(Path(__file__).with_name("decision_policy.md"))
    delivery_contract = read_text(Path(__file__).with_name("delivery_contract.md"))
...
def run(ctx: NodeContext) -> NodeResult:
    ...
    result = run_managed_codex(
        ...
        role="implementation",
        prompt=prompt,
        required_artifacts=["AGENT-STATUS.json", "PISA-SEARCH-STATE.json", "PISA-RECIPES.json"],
        output_path=out,
    )

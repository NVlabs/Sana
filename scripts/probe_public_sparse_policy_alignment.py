#!/usr/bin/env python3
"""Pin public boundaries for local pure sparse-attention policy config.

The seven config covered here are intentionally model-agnostic policy
...
def source_checks() -> dict[str, bool]:
    ...
    return {
        ...
        "local_has_all_policy_modes": all(mode in local for mode in POLICY_CONFIG),
...
def behavior_probe() -> dict[str, Any]:
    ...
    rows: dict[str, Any] = {}
    for idx, (cid, meta) in enumerate(POLICY_CONFIG.items()):
        ...
        rows[cid] = {
            "manifest": str(ROOT / "config" / "sparse_attention" / f"{cid}.toml"),
    ...
    return {
        "config_manifest_alignment": rows,

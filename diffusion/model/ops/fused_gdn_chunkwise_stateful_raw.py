"""Raw (num, den) wrapper around the chunkwise stateful GDN autograd Function.

Why this exists
---------------

``FusedGDNChunkwiseStatefulFunction`` (in ``fused_gdn_chunkwise_stateful_bwd``)
returns the already-divided output ``out = num / (den + eps)``. This is the
right thing for any standalone use, but the bidirectional combine in
``_forward_main_branch_with_cache`` needs the raw numerator and denominator
separately:

    num_fwd, den_fwd = fused_gdn_func(..., direction=1, init_state_kv=cache, ...)
    num_bwd, den_bwd = fused_gdn_func(..., direction=2, ...)
    total = (num_fwd + num_bwd) / (den_fwd + den_bwd + eps)

This wrapper provides the same chunkwise stateful Function but skips the
final divide on forward AND skips ``output_divide_bwd`` on backward —
the caller is responsible for dividing outside.
...
Returning (num, den) in their native dtype (NOT casting to qkv.dtype) matches
the production raw stateful path at
``diffusion.model.ops.fused_gdn_chunkwise.fused_gdn_stateful_chunkwise``
(see lines ~1641-1652) and preserves precision through the caller's bidi sum
+ divide: in a mixed-precision setting (bf16 qkv with fp32 dot_precision),
the caller's combine ``(num_fwd + num_rev) / (den_fwd + den_rev + eps)`` runs
in fp32 before the final cast to qkv.dtype.

Backward accepts ``(dnum, dden[, dfinal_kv, dfinal_z])`` directly — no
implicit divide-VJP. From there the math is bit-for-bit identical to the
divided Function's backward (same Phase B̄ / combine / Phase Ā / RoPE+RMSNorm
VJPs), so we share the helpers via import to avoid duplication.
"""
...
class FusedGDNChunkwiseStatefulRawFunction(torch.autograd.Function):
    ...
    See module docstring for shape contract. All preprocessing (RMSNorm,
    ReLU, RoPE), the Phase A/B/C kernels, and the backward Phase Ā / B̄ /
    combine / norm VJPs are identical to ``FusedGDNChunkwiseStatefulFunction``;
    only the final ``out = num / (den + eps)`` is dropped from forward, and
    correspondingly ``output_divide_bwd`` is dropped from backward (since
    ``dnum`` and ``dden`` flow in directly from grad_outputs).
    """
    ...
    def forward(
        ...
        # dot_precision >= 1, bf16 otherwise) — matches the production
...
def fused_gdn_chunkwise_stateful_raw_autograd(
    ...
    *,
    ...
    Same forward semantics as ``fused_gdn_chunkwise_stateful_autograd`` (incl.
    ``direction``, ``init_state_*``, ``save_final_state`` constraints) but
    skips the final ``out = num / (den + eps)`` divide; the caller is
    responsible for combining ``num`` / ``den`` across directions.

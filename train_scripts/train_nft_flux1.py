...
def _rollout_for_one_prompt(
    ...
    if preview_step > 0:
        ...
        if _can_swap and fullrollout_model_key != preview_model_key:
            _swap_pipeline_model(pipeline, fullrollout_model_key, inference_models, transformer_ddp, original_transformer)
...
def main(_):
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        gathered_rewards_dict = {
            key: gather_tensor_to_all(value, world_size).numpy()
            for key, value in collated_samples["rewards"].items()
        }
        if is_main_process(rank):

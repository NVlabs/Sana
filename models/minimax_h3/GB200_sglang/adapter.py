...
def _validate_sparse_policy(tau: float, thresh_type: str) -> str:
    ...
    if expected is None:
        raise RuntimeError(f"unknown MiniMax-H3 policy {name!r}")
    actual = (
        tau,
        thresh_type,
    ...
    if actual != expected:
        raise RuntimeError(
            f"MiniMax-H3 policy {name!r} requires {expected}, got {actual}"
        )
    return name
...
def _sparse_varlen(
    ...
    *,
    ...
    if os.getenv("H3_SOL_SINK_MODE", "prefix") != "prefix":
        raise RuntimeError("MiniMax-H3 requires the full prefix sink")
    ...
    for raw_start, raw_end in zip(boundaries, boundaries[1:]):
        ...
        if end <= start:
            continue
        if start != 0:
            raise RuntimeError("MiniMax-H3 path expects one live packed document")

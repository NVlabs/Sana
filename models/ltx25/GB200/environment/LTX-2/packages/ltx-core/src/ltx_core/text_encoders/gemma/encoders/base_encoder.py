...
from ltx_core.text_encoders.gemma.tokenizer import LTXGemmaTokenizer
...
# Each Processor.get_attributes() name maps to the Auto* class that loads it from disk.
# The tokenizer slot is filled by our pre-loaded LTXGemmaTokenizer, so it's absent here.
_AUTO_LOADERS = {
    "image_processor": lambda root: AutoImageProcessor.from_pretrained(root, local_files_only=True, use_fast=False),
    "feature_extractor": lambda root: AutoFeatureExtractor.from_pretrained(root, local_files_only=True),
    "video_processor": lambda root: AutoVideoProcessor.from_pretrained(root, local_files_only=True),
}

_TOKENIZER_MAX_LENGTH = 1024


def resolve_gemma_processor_config(gemma_root: str) -> tuple[str, str, type[ProcessorMixin]]:
    """Resolve (tokenizer dir, processor dir, Processor class) from a Gemma model root."""
    tokenizer_root = str(find_matching_file(gemma_root, "tokenizer_config.json").parent)
    processor_cfg_path = find_matching_file(gemma_root, "processor_config.json")
    processor_root = str(processor_cfg_path.parent)
    processor_class = getattr(transformers, json.loads(processor_cfg_path.read_text())["processor_class"])
    return tokenizer_root, processor_root, processor_class


def build_gemma_tokenizer(tokenizer_root: str) -> LTXGemmaTokenizer:
    return LTXGemmaTokenizer(tokenizer_root, _TOKENIZER_MAX_LENGTH)


def build_gemma_processor(
    processor_class: type[ProcessorMixin], processor_root: str, hf_tokenizer: object
) -> ProcessorMixin:
    """Assemble the Gemma Processor from its declared sub-components.
    Introspect what sub-components this Processor class declares via get_attributes() and load
    each via the matching Auto* loader on the processor's directory. Layout-agnostic: works for
    canonical flat layouts and split variants like _readout_proj.
    """
    components: dict[str, object] = {"tokenizer": hf_tokenizer}
    for name in processor_class.get_attributes():
        if name in components:
            continue
        components[name] = _AUTO_LOADERS[name](processor_root)
    return processor_class(**components)


def module_ops_from_gemma_root(gemma_root: str) -> tuple[ModuleOps, ...]:
    tokenizer_root, processor_root, processor_class = resolve_gemma_processor_config(gemma_root)

    def load_tokenizer(module: LTXGemmaTextEncoder) -> LTXGemmaTextEncoder:
        module.tokenizer = build_gemma_tokenizer(tokenizer_root)
        return module
    ...
    def load_processor(module: LTXGemmaTextEncoder) -> LTXGemmaTextEncoder:
        ...
        module.processor = build_gemma_processor(processor_class, processor_root, module.tokenizer.tokenizer)
        return module

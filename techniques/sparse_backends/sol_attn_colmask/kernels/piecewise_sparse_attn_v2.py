...
def _validate_tau(thresh):
    ...
    if not math.isfinite(thresh):
        raise ValueError(f"PISA2 thresh/tau must be finite, got {thresh!r}")
    if 0.0 <= thresh < 1.0 and os.environ.get("PISA2_ALLOW_LOW_TAU", "0") != "1":
        raise ValueError(
            f"PISA2 thresh/tau={thresh} looks like a target density, not a calibrated tau. "
            "Run calibrate_density and pass its threshold, or set PISA2_ALLOW_LOW_TAU=1 "
...
def online_piecewise_sparse_attention(q, k, v, thresh=1.0, block_size=64, scale=None, density=None):
    if density is not None:
        raise ValueError(
            "online_piecewise_sparse_attention no longer accepts density= as an alias for thresh. "
            "PISA2 density is controlled by runtime tau calibration: call calibrate_density(...), "

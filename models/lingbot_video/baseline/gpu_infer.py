...
def attention_kernel() -> str:
    ...
    if value != "fa2":
        raise SystemExit(
            f"[lingbot_video] the physically isolated baseline requires fa2, got {value!r}"
        )
    return value
...
def build_command(
    ...
    if active_experiments:
        raise SystemExit(
            "[lingbot_video] the registered baseline forbids unrelated experimental "
    ...
    if config_id == "lingbot_video_baseline":
        valid_variant = (
            cp_degree == nproc
            and fsdp_enabled
    ...
    elif config_id == "lingbot_video_fsdp4_reference":
        valid_variant = (
            cp_degree == 1
            and fsdp_enabled
            and not ulysses_requested
            and not batch_cfg_enabled
            and not refiner_batch_cfg_enabled
        )
    else:
        raise SystemExit(f"[lingbot_video] unsupported baseline runtime config: {config_id}")
    if not valid_variant:
        raise SystemExit(
            f"[lingbot_video] runtime topology does not match registered config {config_id}"
        )

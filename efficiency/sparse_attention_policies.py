...
#
# These helpers describe which key/value blocks a sparse-attention candidate
...
def minference_dynamic_pattern_bank_mask(
    ...
    *,
    ...
    candidate_masks = torch.stack((a_shape, vertical_slash, block_sparse), dim=0)
    ...
    for pattern_idx, pattern_mask in enumerate(candidate_masks):
    ...
    mask = torch.zeros((bsz, heads, n, n), dtype=torch.bool, device=q.device)
    for pattern_idx in range(candidate_masks.shape[0]):
        mask = torch.where(
            (best == pattern_idx).view(bsz, heads, 1, 1),
            candidate_masks[pattern_idx],
            mask,
        )
    ...
    *,
    ...
    for q_idx in range(nq):
        ...
        if role == "temporal":
            ...
            if not candidates:
                candidates = [min(nk - 1, q_idx)]
            order = sorted(candidates, key=lambda idx: abs((idx // frame_size) - q_frame))
            chosen = order[:keep]
        else:

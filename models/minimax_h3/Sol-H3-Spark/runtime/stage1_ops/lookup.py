...
def install_exact_t2va_lookup(model, pipe):
    ...
    plan = t2va_lookup_plan(*schedules)
    ...
    record = {"mode": "native_T2VA_exact_merged_BF16_four_update_7_row_lookup",
    ...
    class FixedSchedule(torch.nn.Module):
        def __init__(self):
            ...
            for step, row in enumerate(plan):
                self.register_buffer(f"indices_{step}", torch.arange(offset, offset + len(row),
        ...
        def forward(self, timestep):
            ...
            step = require_lookup_step(timestep.detach().cpu().tolist(), plan, record["calls"])
            record["calls"] += 1
            return getattr(self, f"indices_{step}")
    ...
    with torch.inference_mode():
        ...
        for name, target in targets:
            ...
            passed = all(torch.equal(replacement(getattr(lookup, f"indices_{step}"))[0], output)
                         and bool(torch.isfinite(output).all()) for step, output in enumerate(reference))
            if not passed:
            ...
            record["verification"].append({"name": name, "step_rows": [1, 2, 2, 2],

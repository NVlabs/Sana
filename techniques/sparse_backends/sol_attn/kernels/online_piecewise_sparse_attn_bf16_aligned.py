...
"""Pure-BF16 Triton PISA2 with canonical global-threshold routing.
...
``GROUP_SIZE`` and does not match the current PISA2 selector.  This module
keeps the prototype's BF16 Q/K/Kc tensor-core dataplane, but uses the current
PISA2 global diagonal threshold and mandatory local-neighbour exact mask.
...
from kernels import online_piecewise_sparse_attn_bf16_legacy as legacy
from kernels import piecewise_sparse_attn_v2 as canonical_pisa2
...
def _validate_inputs(
    ...
    if q.dtype != torch.bfloat16 or k.dtype != torch.bfloat16:
        raise TypeError(
            "pure-BF16 PISA2 requires BF16 Q/K, got "
            f"q={q.dtype} k={k.dtype}"
        )
    if v.dtype != torch.bfloat16:
        raise TypeError(f"pure-BF16 PISA2 requires BF16 V, got {v.dtype}")
    if q.device.type != "cuda" or k.device != q.device or v.device != q.device:
        raise ValueError("pure-BF16 PISA2 requires Q/K/V on one CUDA device")
    if q.ndim != 4 or k.shape != q.shape or v.shape != q.shape:
        raise ValueError(
            "pure-BF16 PISA2 currently requires equal [B,H,T,D] Q/K/V, got "
            f"q={tuple(q.shape)} k={tuple(k.shape)} v={tuple(v.shape)}"
        )
    if q.shape[-1] != 128:
        raise ValueError(f"pure-BF16 PISA2 currently requires D=128, got {q.shape[-1]}")
    if isinstance(block_size, bool) or block_size != 64:
        raise ValueError(f"pure-BF16 PISA2 currently requires block_size=64, got {block_size!r}")
...
def prepare_qkv(
    ...
    *,
    ...
    global_thresh = canonical_pisa2.compute_global_qck_threshold(
        q,
        unit_scale,
        kc,
        unit_scale,
...
def single_pass_dynamic_routing_kernel(
    ...
    """Fused BF16 attention main with canonical PISA2 selector semantics."""
...
def online_piecewise_sparse_attention(
    ...
    *,
    ...
    """Run complete pure-BF16, routing-aligned PISA2 forward."""
...
__all__ = [
    ...
    "online_piecewise_sparse_attention",
    "prepare_qkv",
]
